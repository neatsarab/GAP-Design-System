  ...
  <div>
    ...
    <div class="d-flex align-center justify-space-between mb-5 flex-wrap ga-3">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis mb-0">รายการใบรับรองการจดทะเบียนผู้ส่งออกที่ได้รับการอนุมัติ</p>
      </div>
      <v-btn variant="tonal" color="export-user" prepend-icon="fas fa-download">ส่งออก Excel</v-btn>
    </div>
    ...
    <v-chip-group v-model="activeTab" class="mb-4" mandatory>
      <v-chip value="all" color="export-user" variant="tonal" filter size="small">
        <v-icon start icon="fas fa-list" size="12" /> ทั้งหมด
        <v-badge :content="allItems.length" inline color="export-user" class="ml-1" />
      </v-chip>
    ...
    <v-card rounded="xl" elevation="0" class="data-card">
      <v-data-table :headers="headers" :items="filteredItems" :search="search" hover>
        <template #item.requestNo="{ item }">
          <span class="text-body-2 font-weight-medium text-export-user">{{ item.requestNo }}</span>
        </template>
        ...
        <template #item.actions="{ item }">
          <div class="d-flex ga-1">
            <v-btn size="small" variant="text" color="export-user" icon="fas fa-eye" />
...
const stats = computed(() => [
  { label: "ทั้งหมด",      icon: "fas fa-file-certificate", color: "export-user", value: allItems.length },
