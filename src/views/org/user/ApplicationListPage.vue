  ...
  <div>
    <div class="d-flex align-center justify-space-between mb-5 flex-wrap ga-3">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis mb-0">ติดตามสถานะคำขอรับรองมาตรฐานเกษตรอินทรีย์ทั้งหมด</p>
      </div>
      <v-btn color="org-user" prepend-icon="fas fa-file-pen" @click="router.push('/org/user/applications/new')">
        ยื่นคำขอใหม่
      </v-btn>
    </div>
      ...
      <v-card-text class="pa-4">
        <v-row dense align="center">
          <v-col cols="12" sm="5">
            ...
            <v-text-field v-model="search" placeholder="ค้นหาเลขที่คำขอ / ชื่อแปลง" prepend-inner-icon="fas fa-search" variant="outlined" density="compact" rounded="lg" clearable hide-details />
          </v-col>
          <v-col cols="12" sm="3">
    ...
    <v-chip-group v-model="activeTab" class="mb-4" mandatory>
      <v-chip v-for="tab in statusTabs" :key="tab.value" :value="tab.value" :color="tab.color" variant="tonal" filter size="small">
        ...
        {{ tab.label }}
        <v-badge :content="countByStatus(tab.value)" inline :color="tab.color" class="ml-1" />
      </v-chip>
    </v-chip-group>
    ...
    <v-card rounded="xl" elevation="0">
      <v-data-table :headers="headers" :items="filteredItems" :search="search" hover @click:row="onRowClick">
        <template #item.requestNo="{ item }">
          <span class="text-body-2 font-weight-bold text-org-user">{{ item.requestNo }}</span>
        </template>
        <template #item.type="{ item }">
          <v-chip size="x-small" :color="item.type === 'individual' ? 'org-user' : 'secondary'" variant="tonal">
        ...
        <template #item.actions="{ item }">
          <v-btn size="small" variant="tonal" color="org-user" prepend-icon="fas fa-eye" @click.stop="router.push(`/org/user/applications/${item.id}`)">
            ดูรายละเอียด
          </v-btn>
        </template>
        <template #no-data>
          <div class="pa-10 text-center">
            <v-icon icon="fas fa-leaf" size="48" color="org-user" style="opacity:0.2" />
...
const statusTabs = [
  ...
  { value: 'approved',          label: 'อนุมัติแล้ว',            color: 'success',   icon: 'fas fa-circle-check' },
