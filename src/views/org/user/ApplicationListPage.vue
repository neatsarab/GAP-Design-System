<template>
    <div>
        <div class="d-flex align-center justify-space-between mb-6 flex-wrap ga-3">
            ...
            <v-btn color="org-user" prepend-icon="fas fa-plus" @click="goToNewApplication">
                ยื่นคำขอใหม่
            </v-btn>
        </div>
            ...
            <v-card-text class="pa-4">
                <v-row dense align="center">
                    <v-col cols="12" sm="6" md="6">
                        ...
                        <v-text-field v-model="search" placeholder="ค้นหาเลขคำขอ" prepend-inner-icon="fas fa-search"
                    ...
                    <v-col cols="12" sm="6" md="3">
                        ...
                        <v-menu v-model="dateFromMenu" :close-on-content-click="false" location="bottom start">
                            ...
                            <v-date-picker v-model="dateFromObj" color="org-user" show-adjacent-months
                    ...
                    <v-col cols="12" sm="6" md="3">
                        ...
                        <v-menu v-model="dateToMenu" :close-on-content-click="false" location="bottom start">
                            ...
                            <v-date-picker v-model="dateToObj" color="org-user" show-adjacent-months
                ...
                <template #item.actions="{ item }">
                    <div class="d-flex align-center ga-1">
                        <v-tooltip v-if="item.note" text="ดูหมายเหตุ" location="top">
                            <template #activator="{ props }">
                                <v-btn v-bind="props" icon size="x-small" variant="text" color="orange"
                                    @click.stop="openNote(item)">
                                    <v-icon icon="fas fa-comment-dots" size="14" />
                                </v-btn>
                            </template>
                        </v-tooltip>
                        <v-btn v-if="item.status === 'need_edit'" size="x-small" variant="tonal" color="warning"
                            prepend-icon="fas fa-pencil" @click.stop="
                                router.push({
                        ...
                        <v-tooltip text="ดูรายละเอียด" location="top">
                            <template #activator="{ props }">
                                <v-btn v-bind="props" icon size="x-small" variant="text" color="org-user"
                                    @click="goToApplicationDetail(item.requestNo)">
                        ...
                        <v-tooltip v-if="item.status === 'draft'" text="แก้ไข" location="top">
                            <template #activator="{ props }">
                                <v-btn v-bind="props" icon size="x-small" variant="text" color="warning" @click.stop="
                                    router.push({
                                        name: 'ORGUserEditApplication',
                                        params: { id: item.requestNo },
                                    })
                        ...
                        <v-tooltip v-if="item.status !== 'approved'" text="ยกเลิก" location="top">
                            <template #activator="{ props }">
                                <v-btn v-bind="props" icon size="x-small" variant="text" color="error" @click.stop>
                                    <v-icon icon="fas fa-xmark" size="14" @click.stop="
                                    router.push({
        ...
        <v-dialog v-model="noteDialog" max-width="480">
            <v-card rounded="xl">
                <v-card-title class="d-flex align-center ga-2 pa-5 pb-3">
                    ...
                    <v-chip v-if="noteItem" size="x-small" variant="tonal" color="org-user" class="ml-1">{{
                        noteItem.requestNo
                        }}</v-chip>
                    <v-spacer />
...
function goToNewApplication() {
    router.push({ name: "GAPUserApplicationType" });
}
...
function goToApplicationDetail(no) {
    router.push({ name: "ORGUserApplicationDetail", params: { id: no } });
}
</script>
