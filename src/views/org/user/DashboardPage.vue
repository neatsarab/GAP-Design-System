<template>
  <div>
    ...
    <v-row>
      <v-col cols="12" md="8">
        <v-card rounded="xl" elevation="0" class="h-100">
          <v-card-title class="pa-5 pb-0 d-flex align-center justify-space-between">
            ...
            <v-btn variant="text" color="org-user" size="small" append-icon="fas fa-arrow-right" @click="router.push('/org/user/applications')">
              ดูทั้งหมด
            </v-btn>
          </v-card-title>
          <v-list lines="two" class="pa-2">
            <v-list-item v-for="app in recentApplications" :key="app.id" rounded="lg" class="mb-1" @click="router.push(`/org/user/applications/${app.id}`)">
              <template #prepend>
              ...
              <v-list-item-subtitle class="text-caption">{{ app.farmName }} · {{ app.area }} ไร่</v-list-item-subtitle>
              <template #append>
      ...
      <v-col cols="12" md="4">
        <v-alert color="org-user" variant="tonal" rounded="xl" class="mb-4" prepend-icon="fas fa-circle-info">
          <div class="text-body-2 font-weight-medium mb-1">อัพเดทสถานะ</div>
          <div class="text-body-2">คำขอ ORG-2568-00002 อยู่ระหว่างการตรวจประเมินแปลง</div>
        </v-alert>
        <v-card rounded="xl" elevation="0">
          ...
          <v-list density="compact" nav class="px-2 pb-3">
            <v-list-item prepend-icon="fas fa-file-pen" title="ยื่นคำขอ ORG ใหม่" rounded="lg" color="org-user" class="mb-1" @click="router.push('/org/user/applications/new')" />
            <v-list-item prepend-icon="fas fa-file-lines" title="รายการคำขอ" rounded="lg" color="org-user" @click="router.push('/org/user/applications')" />
          </v-list>
        </v-card>
      </v-col>
    </v-row>
...
const stats = [
  { label: "คำขอทั้งหมด", value: 4, icon: "fas fa-file-lines", color: "org-user" },
  { label: "อยู่ระหว่างตรวจสอบ", value: 1, icon: "fas fa-magnifying-glass", color: "org-user" },
...
const recentApplications = [
  { id: "ORG-2568-00003", requestNo: "ORG-2568-00003", farmName: "แปลงสวนอินทรีย์ A", area: "15", submittedDate: "5 มี.ค. 2568", status: "under_review" },
  { id: "ORG-2568-00002", requestNo: "ORG-2568-00002", farmName: "แปลงสวนอินทรีย์ B", area: "8", submittedDate: "20 ก.พ. 2568", status: "inspection_scheduled" },
  { id: "ORG-2568-00001", requestNo: "ORG-2568-00001", farmName: "แปลงสวนอินทรีย์ A", area: "15", submittedDate: "10 ม.ค. 2568", status: "approved" },
];
