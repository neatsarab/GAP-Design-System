  ...
  <div>
    <div class="d-flex align-center justify-space-between mb-5 flex-wrap ga-3">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis mb-0">จัดการคำขอรับรองมาตรฐานเกษตรอินทรีย์ทั้งหมด</p>
      </div>
    </div>
      ...
      <v-card-text class="pa-4">
        <v-row dense align="center">
          <v-col cols="12" sm="4">
            ...
            <v-text-field v-model="search" placeholder="ค้นหาเลขที่คำขอ / ชื่อผู้ยื่น" prepend-inner-icon="fas fa-search" variant="outlined" density="compact" rounded="lg" clearable hide-details />
          </v-col>
          <v-col cols="6" sm="2">
            <div class="field-label mb-1"><div>ประเภท</div><div class="field-label-en">Type</div></div>
            <v-select v-model="filterType" :items="typeOptions" variant="outlined" density="compact" rounded="lg" hide-details clearable />
          </v-col>
          <v-col cols="6" sm="2">
            <div class="field-label mb-1"><div>จังหวัด</div><div class="field-label-en">Province</div></div>
            <v-select v-model="filterProvince" :items="provinceOptions" variant="outlined" density="compact" rounded="lg" hide-details clearable />
          </v-col>
          <v-col cols="12" sm="3">
    ...
    <v-chip-group v-model="activeTab" class="mb-4" mandatory>
      <v-chip v-for="tab in statusTabs" :key="tab.value" :value="tab.value" :color="tab.color" variant="tonal" filter size="small">
        ...
        {{ tab.label }}
        <v-badge v-if="tab.value !== 'all'" :content="countByStatus(tab.value)" inline :color="tab.color" class="ml-1" />
      </v-chip>
    </v-chip-group>
    ...
    <v-card rounded="xl" elevation="0">
      <v-data-table :headers="headers" :items="filteredItems" :search="search" hover @click:row="onRowClick">
        <template #item.requestNo="{ item }">
          <span class="text-body-2 font-weight-bold text-org-staff">{{ item.requestNo }}</span>
        </template>
        <template #item.type="{ item }">
          <v-chip size="x-small" :color="item.type === 'individual' ? 'org-staff' : 'secondary'" variant="tonal">
            <v-icon start :icon="item.type === 'individual' ? 'fas fa-user' : 'fas fa-people-group'" size="10" />
            {{ item.type === 'individual' ? 'รายเดี่ยว' : 'รายกลุ่ม' }}
          </v-chip>
        </template>
        <template #item.status="{ item }">
          <v-chip size="small" :color="statusColor(item.status)" variant="tonal" :prepend-icon="statusIcon(item.status)">
            {{ statusLabel(item.status) }}
          </v-chip>
        </template>
        <template #item.actions="{ item }">
          <v-btn size="small" variant="tonal" color="org-staff" prepend-icon="fas fa-eye" @click.stop="router.push(`/org/staff/applications/${item.id}`)">
            ดำเนินการ
          </v-btn>
        </template>
        <template #no-data>
          <div class="pa-10 text-center">
            <v-icon icon="fas fa-leaf" size="48" color="org-staff" style="opacity:0.2" />
            <p class="text-body-2 text-medium-emphasis mt-3 mb-0">ไม่พบรายการคำขอ</p>
          </div>
        </template>
      </v-data-table>
    </v-card>
...
const provinceOptions = ['เชียงใหม่', 'เชียงราย', 'อุบลราชธานี', 'ยโสธร', 'สุรินทร์', 'เลย']
const statusOptions = [
...
const allApplications: Application[] = [
  ...
  { id: 'ORG-2568-00007', requestNo: 'ORG-2568-00007', applicantName: 'นางสาวรุ่งนภา ฟาร์ม',    farmName: 'แปลงผักอินทรีย์',          crop: 'ผัก',       area: '5',  type: 'individual', province: 'เลย',           submittedDate: '25 ก.พ. 2568', status: 'revision_required' },
  { id: 'ORG-2568-00006', requestNo: 'ORG-2568-00006', applicantName: 'นายประสิทธิ์ มีสุข',      farmName: 'สวนชาอินทรีย์',           crop: 'ชา',        area: '30', type: 'individual', province: 'เชียงใหม่',    submittedDate: '20 ก.พ. 2568', status: 'pending_cc' },
  { id: 'ORG-2568-00005', requestNo: 'ORG-2568-00005', applicantName: 'นางวิมล ทองดี',           farmName: 'แปลงสมุนไพร',              crop: 'สมุนไพร',  area: '12', type: 'individual', province: 'เลย',           submittedDate: '10 ก.พ. 2568', status: 'cc_reviewing' },
  { id: 'ORG-2568-00003', requestNo: 'ORG-2568-00003', applicantName: 'นายสมชาย เกษตรดี',        farmName: 'แปลงข้าวอินทรีย์',        crop: 'ข้าว',      area: '18', type: 'individual', province: 'ยโสธร',         submittedDate: '15 ม.ค. 2568', status: 'approved' },
  { id: 'ORG-GRP-2568-00004', requestNo: 'ORG-GRP-2568-00004', applicantName: 'กลุ่มเกษตรอินทรีย์บ้านโนน', farmName: 'กลุ่มผักรวม', crop: 'ผักรวม', area: '120', type: 'group', province: 'อุบลราชธานี', submittedDate: '3 มี.ค. 2568', status: 'submitted' },
...
const filteredItems = computed(() => {
  ...
  if (filterType.value) items = items.filter(i => filterType.value === 'รายเดี่ยว' ? i.type === 'individual' : i.type === 'group')
  if (filterProvince.value) items = items.filter(i => i.province === filterProvince.value)
...
function clearFilters() {
  search.value = ''; filterType.value = null; filterProvince.value = null
  filterStatus.value = null; activeTab.value = 'all'
}
