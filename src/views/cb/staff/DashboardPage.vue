<template>
  <div>
    ...
    <v-row>
      ...
      <v-col cols="12" md="8">
        <v-card rounded="xl" elevation="0">
          <v-card-title class="d-flex align-center ga-2 pa-4 pb-3">
            <v-icon icon="fas fa-clock-rotate-left" color="cb-staff" size="16" />
          ...
          <v-list lines="two" class="pa-0">
            <template v-for="(app, i) in pendingApplications" :key="app.id">
              <v-list-item class="pa-3" @click="router.push(`/cb/staff/applications/${app.id}`)">
                <template #prepend>
      ...
      <v-col cols="12" md="4">
        <v-alert color="cb-staff" variant="tonal" rounded="xl" class="mb-4" prepend-icon="fas fa-circle-info">
          <div class="text-body-2 font-weight-medium mb-1">สรุปงานที่รอดำเนินการ</div>
          <div class="text-body-2">มีคำขอรอดำเนินการทั้งหมด 3 รายการ รวมถึงการตรวจประเมิน CB 2 รายการ</div>
        </v-alert>
        <!-- Status Summary -->
...
const stats = [
  { label: "คำขอทั้งหมด", value: 15, icon: "fas fa-file-lines", iconColor: "cb-staff" },
  { label: "รอตรวจสอบ", value: 3, icon: "fas fa-clock", iconColor: "warning" },
  { label: "อยู่ระหว่างตรวจประเมิน", value: 2, icon: "fas fa-magnifying-glass", iconColor: "info" },
  { label: "อนุมัติแล้ว", value: 9, icon: "fas fa-circle-check", iconColor: "success" },
];
