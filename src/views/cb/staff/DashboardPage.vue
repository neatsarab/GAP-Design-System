<template>
  <div>
    <div class="d-flex align-center justify-space-between mb-6 flex-wrap ga-3">
      <div>
        <h1 class="text-h5 font-weight-bold mb-1">แดชบอร์ด CB (เจ้าหน้าที่)</h1>
        <p class="text-body-2 text-medium-emphasis mb-0">ภาพรวมคำขอขึ้นทะเบียนหน่วยรับรองทั้งหมด</p>
      </div>
    </div>
      ...
      <v-col cols="12" md="8">
        <v-card rounded="xl" elevation="0" class="h-100">
          <v-card-title class="pa-5 pb-0 d-flex align-center justify-space-between">
            ...
            <v-btn variant="text" color="cb-staff" size="small" append-icon="fas fa-arrow-right" @click="router.push('/cb/staff/applications')">
              ดูทั้งหมด
            </v-btn>
          </v-card-title>
          <v-list lines="two" class="pa-2">
            <v-list-item v-for="app in pendingApplications" :key="app.id" rounded="lg" class="mb-1" @click="router.push(`/cb/staff/applications/${app.id}`)">
              <template #prepend>
      ...
      <v-col cols="12" md="4">
        <v-card rounded="xl" elevation="0">
          ...
          <v-list density="compact" nav class="px-2 pb-3">
            <v-list-item prepend-icon="fas fa-file-lines" title="รายการคำขอทั้งหมด" rounded="lg" color="cb-staff" class="mb-1" @click="router.push('/cb/staff/applications')" />
            <v-list-item prepend-icon="fas fa-magnifying-glass" title="ตรวจประเมิน CB" rounded="lg" color="cb-staff" @click="router.push('/cb/staff/inspection')" />
          </v-list>
        </v-card>
      </v-col>
    </v-row>
...
const stats = [
  { label: "คำขอทั้งหมด", value: 15, icon: "fas fa-file-lines", color: "cb-staff" },
  { label: "รอตรวจสอบ", value: 3, icon: "fas fa-clock", color: "warning" },
  { label: "อยู่ระหว่างตรวจประเมิน", value: 2, icon: "fas fa-magnifying-glass", color: "info" },
  { label: "อนุมัติแล้ว", value: 9, icon: "fas fa-circle-check", color: "success" },
];

const pendingApplications = [
  { id: "CB-2568-00008", requestNo: "CB-2568-00008", cbName: "บ.เอเชียเซอร์ติฟาย จก.", scope: "ผลไม้ส่งออก", submittedDate: "9 มี.ค. 2568", status: "submitted" },
  { id: "CB-2568-00007", requestNo: "CB-2568-00007", cbName: "บ.ไทยมาตรฐาน จก.", scope: "ผักและสมุนไพร", submittedDate: "6 มี.ค. 2568", status: "under_review" },
  { id: "CB-2568-00006", requestNo: "CB-2568-00006", cbName: "บ.กรีนเซอร์ติฟาย จก.", scope: "ธัญพืชอินทรีย์", submittedDate: "2 มี.ค. 2568", status: "inspection_scheduled" },
];
