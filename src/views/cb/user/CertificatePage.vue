  ...
  <div>
    ...
    <div class="d-flex align-center justify-space-between mb-5 flex-wrap ga-3">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis mb-0">รายการใบรับรองหน่วยรับรองโรงงานผลิตสินค้าพืชของคุณ</p>
      </div>
      <v-btn variant="tonal" color="cb-user" prepend-icon="fas fa-download">ส่งออก Excel</v-btn>
    </div>
      ...
      <v-card-text class="pa-4">
        <v-row dense align="center">
          ...
          <v-col cols="6" sm="3">
            ...
            <v-autocomplete v-model="filterCertType" :items="certTypeOptions" placeholder="ทั้งหมด" variant="outlined" density="compact" rounded="lg" hide-details clearable />
          </v-col>
    ...
    <v-chip-group v-model="activeTab" class="mb-4" mandatory>
      <v-chip value="all" color="cb-user" variant="tonal" filter size="small">
        ...
        <v-badge :content="allItems.length" inline color="cb-user" class="ml-1" />
      </v-chip>
    ...
    <v-card rounded="xl" elevation="0" class="data-card">
      <v-data-table :headers="headers" :items="filteredItems" :search="search" hover>
        <template #item.cbName="{ item }">
          <span class="text-body-2 font-weight-medium text-cb-user">{{ item.cbName }}</span>
        </template>
        ...
        <template #item.actions="{ item }">
          <div class="d-flex ga-1">
            <v-btn size="small" variant="text" color="cb-user" icon="fas fa-eye" @click.stop="viewCert(item)" />
            <v-btn size="small" variant="text" color="success" icon="fas fa-download" :disabled="item.status === 'expired'" @click.stop="downloadCert(item)" />
          </div>
        </template>
      </v-data-table>
    </v-card>
    ...
    <v-dialog v-model="viewDialog" max-width="560">
      <v-card rounded="xl">
        <v-card-title class="pa-5 pb-3 d-flex align-center justify-space-between text-body-1 font-weight-bold">
          <span><v-icon icon="fas fa-certificate" color="cb-user" class="mr-2" size="18" />รายละเอียดใบรับรอง</span>
          <v-chip v-if="selectedCert" :color="statusColor(selectedCert.status)" size="small" variant="tonal">{{ statusLabel(selectedCert.status) }}</v-chip>
        </v-card-title>
        <v-divider />
        <v-card-text v-if="selectedCert" class="pa-5">
          <v-row dense>
            <v-col cols="12" sm="6"><div class="info-label">ประเภท</div><div class="info-value">{{ selectedCert.certType }}</div></v-col>
            <v-col cols="12"><div class="info-label">ชื่อหน่วยรับรอง</div><div class="info-value">{{ selectedCert.cbName }}</div></v-col>
            <v-col cols="12" sm="6"><div class="info-label">วันที่ออก</div><div class="info-value">{{ selectedCert.issueDate }}</div></v-col>
            <v-col cols="12" sm="6"><div class="info-label">วันหมดอายุ</div><div class="info-value">{{ selectedCert.expireDate }}</div></v-col>
          </v-row>
        </v-card-text>
...
const stats = computed(() => [
  { label: "ทั้งหมด",      icon: "fas fa-certificate",  color: "cb-user", value: allItems.length },
...
const filteredItems = computed(() => {
  ...
  if (filterCertType.value) items = items.filter((i) => i.certType === filterCertType.value);
  return items;
});
