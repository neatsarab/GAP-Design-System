<template>
  <div>
    ...
    <v-tabs v-model="activeTab" color="hcex-staff" class="mb-4">
      ...
      <v-tab value="factories" prepend-icon="fas fa-industry"
        >โรงงาน/สถานที่ผลิต</v-tab
      >
    </v-tabs>
      ...
      <v-tabs-window-item value="exporters">
            ...
            <v-row dense align="center">
              <v-col cols="12" sm="5">
                ...
                <v-text-field
                  ...
                  placeholder="ค้นหาชื่อบริษัท / เลขทะเบียน"
        ...
        <v-chip-group v-model="exporterTabFilter" class="mb-4" mandatory>
          ...
          >
            ...
            <v-badge
              :content="countExporterByStatus('active')"
              inline
              color="success"
              class="ml-1"
            />
          ...
          >
            ...
            <v-badge
              :content="countExporterByStatus('expiring')"
              inline
              color="warning"
              class="ml-1"
            />
          ...
          >
            ...
            <v-badge
              :content="countExporterByStatus('expired')"
              inline
              color="error"
              class="ml-1"
            />
      ...
      <v-tabs-window-item value="factories">
            ...
            <v-row dense align="center">
              <v-col cols="12" sm="5">
                ...
                <v-text-field
                  ...
                  placeholder="ค้นหาชื่อโรงงาน / เลขทะเบียน"
                  prepend-inner-icon="fas fa-search"
                  variant="outlined"
                  density="compact"
                  rounded="lg"
        ...
        <v-chip-group v-model="factoryTabFilter" class="mb-4" mandatory>
          ...
          >
            <v-icon start icon="fas fa-circle-check" size="12" /> มีผล
            <v-badge
              :content="countFactoryByStatus('active')"
              inline
              color="success"
              class="ml-1"
            />
          ...
          >
            <v-icon start icon="fas fa-clock" size="12" /> ใกล้หมดอายุ
            <v-badge
              :content="countFactoryByStatus('expiring')"
              inline
              color="warning"
              class="ml-1"
            />
          ...
          >
            <v-icon start icon="fas fa-circle-xmark" size="12" /> หมดอายุ
            <v-badge
              :content="countFactoryByStatus('expired')"
              inline
              color="error"
              class="ml-1"
            />
...
const exporters = [
  {
    id: "exp-001",
    regNo: "EXP-2569-00001",
    companyName: "บริษัท ไทยฟู้ดโปรเซส จำกัด",
    taxId: "0105560012345",
    registeredDate: "01/01/2569",
    expireDate: "01/01/2571",
    certStatus: "active",
  },
  {
    id: "exp-002",
    regNo: "EXP-2569-00002",
    companyName: "บริษัท เกษตรแปรรูปไทย จำกัด",
    taxId: "0105561023456",
    registeredDate: "15/02/2569",
    expireDate: "15/02/2571",
    certStatus: "active",
  },
  {
    id: "exp-003",
    regNo: "EXP-2569-00003",
    companyName: "ห้างหุ้นส่วน สินทวีผลผลิต",
    taxId: "0993562034567",
    registeredDate: "10/06/2569",
    expireDate: "01/04/2569",
    certStatus: "expiring",
  },
  {
    id: "exp-004",
    regNo: "EXP-2569-00004",
    companyName: "บริษัท อาหารส่งออกไทย จำกัด",
    taxId: "0105563045678",
    registeredDate: "20/09/2569",
    expireDate: "20/09/2571",
    certStatus: "active",
  },
  {
    id: "exp-005",
    regNo: "EXP-2569-00098",
    companyName: "บริษัท สยามผลผลิตพืช จำกัด",
    taxId: "0105554056789",
    registeredDate: "01/01/2565",
...
const stats = computed(() => [
  {
    ...
    color: "primary",
    value: exporters.length + factories.length,
  },
  {
    label: "มีผล",
    icon: "fas fa-circle-check",
    color: "success",
    value: countExporterByStatus("active") + countFactoryByStatus("active"),
  },
  {
    label: "ใกล้หมดอายุ",
    icon: "fas fa-clock",
    color: "warning",
    value: countExporterByStatus("expiring") + countFactoryByStatus("expiring"),
  },
  {
    label: "หมดอายุ",
    icon: "fas fa-circle-xmark",
    color: "error",
    value: countExporterByStatus("expired") + countFactoryByStatus("expired"),
  },
]);

function countExporterByStatus(s) {
  return exporters.filter((i) => i.certStatus === s).length;
}
function countFactoryByStatus(s) {
  return factories.filter((i) => i.certStatus === s).length;
}

const filteredExporters = computed(() => {
  let items = exporters;
  if (exporterTabFilter.value !== "all")
    items = items.filter((i) => i.certStatus === exporterTabFilter.value);
...
const filteredFactories = computed(() => {
  let items = factories;
  if (factoryTabFilter.value !== "all")
    items = items.filter((i) => i.certStatus === factoryTabFilter.value);
