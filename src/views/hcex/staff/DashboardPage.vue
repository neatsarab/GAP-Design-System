  ...
  <div>
    <div class="d-flex align-center justify-space-between flex-wrap ga-3 mb-6">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis mb-0">ภาพรวมระบบออกใบรับรองสุขอนามัย (Health Certificate) สินค้าแปรรูปด้านพืช</p>
      </div>
    </div>
    ...
    <v-row>
      ...
      <v-col cols="12" md="8">
        <v-card rounded="xl" elevation="0">
          <v-card-title class="d-flex align-center ga-2 pa-4 pb-3">
            <v-icon icon="fas fa-clock-rotate-left" color="hcex-staff" size="16" />
            <span class="text-body-1 font-weight-bold">คำขอล่าสุด</span>
            <v-spacer />
            <v-btn variant="text" color="hcex-staff" size="small" append-icon="fas fa-arrow-right" @click="router.push('/hcex/staff/applications')">
              ดูทั้งหมด
            </v-btn>
          </v-card-title>
          <v-divider />
          <v-list lines="two" class="pa-0">
            <template v-for="(app, i) in recentApplications" :key="app.id">
              <v-list-item class="pa-3" @click="router.push(`/hcex/staff/applications/${app.id}`)">
                <template #prepend>
                ...
                <v-list-item-subtitle class="text-caption">{{ app.exporter }} · {{ app.certType }}</v-list-item-subtitle>
                <template #append>
      ...
      <v-col cols="12" md="4">
        <v-alert color="hcex-staff" variant="tonal" rounded="xl" class="mb-4" prepend-icon="fas fa-circle-info">
          <div class="text-body-2 font-weight-medium mb-1">สรุปงานที่รอดำเนินการ</div>
          <div class="text-body-2">มีคำขอรอดำเนินการ 9 รายการ รวมถึงรอพิจารณา Lab 5 รายการ</div>
        </v-alert>
        <!-- Status Summary -->
...
const recentApplications = [
  { id: "HCEX-001", requestNo: "HCEX-2568-00012", exporter: "บ.ไทยฟู้ดโปรเซส จก.", certType: "กมพ.1", submittedAt: "10 ม.ค. 68", status: "under_review" },
  { id: "HCEX-002", requestNo: "HCEX-2568-00011", exporter: "บ.สยามแปรรูป จก.", certType: "กมพ.1-1", submittedAt: "9 ม.ค. 68", status: "lab_verification" },
  { id: "HCEX-003", requestNo: "HCEX-2568-00010", exporter: "บ.กรีนโปรดักส์ จก.", certType: "กมพ.1-2", submittedAt: "7 ม.ค. 68", status: "pending_signing" },
  { id: "HCEX-004", requestNo: "HCEX-2568-00009", exporter: "บ.ดอยอาหาร จก.", certType: "กมพ.1", submittedAt: "5 ม.ค. 68", status: "completed" },
  { id: "HCEX-005", requestNo: "HCEX-2568-00008", exporter: "บ.เอเชียฟู้ดส์ จก.", certType: "กมพ.1-3", submittedAt: "3 ม.ค. 68", status: "completed" },
];
...
function getStatusColor(s: string) {
  const m: Record<string, string> = {
    draft: "grey", submitted: "warning", under_review: "info",
    lab_verification: "secondary", pending_signing: "primary",
...
function getStatusLabel(s: string) {
  const m: Record<string, string> = {
    ...
    lab_verification: "รอพิจารณา Lab", pending_signing: "รอลงนาม",
