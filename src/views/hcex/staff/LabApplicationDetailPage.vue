        ...
        <div class="d-flex align-center">
          <template v-for="(step, i) in timelineSteps" :key="step.value">
            ...
            <div
              ...
              :class="{ 'step-line--done': application.currentStep > step.value }"
            />
          </template>
        </div>
      </v-card-text>
    ...
    <v-row>
      ...
      <v-col cols="12" md="8">
        <v-tabs v-model="activeTab" color="hcex-staff" class="mb-4">
          ...
          <v-tab value="review" prepend-icon="fas fa-flask">บันทึกผลการพิจารณา</v-tab>
        </v-tabs>
          ...
          <v-window-item value="info">
                ...
                <v-row dense>
                  ...
                  <v-col cols="6" md="4">
                    ...
                    <div class="info-value">
                      <v-chip size="x-small" :color="objectiveColor(application.objective)" variant="tonal">
            ...
            <v-card rounded="xl" elevation="0" class="section-card mb-4">
              <div class="section-header px-4 py-3 border-b d-flex align-center ga-2">
                ...
                <span class="text-subtitle-2 font-weight-bold">ข้อมูลตัวอย่างและสินค้า</span>
              </div>
              <v-card-text class="pa-4">
                <v-row dense>
                  <v-col cols="12" md="6">
                    <div class="info-label">ชื่อผลิตภัณฑ์ (ภาษาไทย)</div>
                    <div class="info-value">{{ application.sample.productNameTh }}</div>
                  </v-col>
                  <v-col cols="12" md="6">
                    <div class="info-label">Product Name (English)</div>
                    <div class="info-value">{{ application.sample.productNameEn }}</div>
                  </v-col>
                  <v-col cols="12" md="4">
                    <div class="info-label">รหัสผลิตภัณฑ์ / Product Code</div>
                    <div class="info-value text-hcex-staff font-weight-bold">{{ application.sample.productCode }}</div>
                  </v-col>
                  <v-col cols="12" md="4">
                    <div class="info-label">วันที่ผลิต / MFG Date</div>
                    <div class="info-value">{{ application.sample.mfgDate }}</div>
                  </v-col>
                  <v-col cols="12" md="4">
                    <div class="info-label">วันหมดอายุ / EXP Date</div>
                    <div class="info-value">{{ application.sample.expDate }}</div>
                  </v-col>
                  <v-col cols="12" md="6">
                    <div class="info-label">วันที่สุ่มเก็บตัวอย่าง / Date Collected</div>
                    <div class="info-value">{{ application.sample.dateCollected }}</div>
                  </v-col>
                  <v-col cols="12" md="6">
                    <div class="info-label">วันที่นำส่งตัวอย่าง / Date Delivered</div>
                    <div class="info-value">{{ application.sample.dateDelivered }}</div>
                  </v-col>
                </v-row>
              </v-card-text>
            </v-card>
            ...
            <v-card rounded="xl" elevation="0" class="section-card mb-4">
              <div class="section-header px-4 py-3 border-b d-flex align-center ga-2">
                ...
                <span class="text-subtitle-2 font-weight-bold">สถานที่ผู้ผลิต (Manufacturer)</span>
              </div>
              <v-card-text class="pa-4">
                <v-row dense>
                  <v-col cols="12" md="6">
                    <div class="info-label">ผู้ผลิต / Manufacturer</div>
                    <div class="info-value font-weight-medium">{{ application.manufacturer.name }}</div>
                  </v-col>
                  <v-col cols="12" md="6">
                    <div class="info-label">ที่อยู่โรงงาน / Factory Address</div>
            ...
            <v-card rounded="xl" elevation="0" class="section-card mb-4">
              <div class="section-header px-4 py-3 border-b d-flex align-center ga-2">
                <v-icon icon="fas fa-location-dot" color="hcex-staff" size="15" />
                <span class="text-subtitle-2 font-weight-bold">สถานที่สุ่มเก็บตัวอย่าง (Collecting Location)</span>
              </div>
              <v-card-text class="pa-4">
                <div class="info-value">{{ application.collectingLocation }}</div>
              </v-card-text>
            </v-card>
            ...
            <v-card rounded="xl" elevation="0" class="section-card mb-4">
              <div class="section-header px-4 py-3 border-b d-flex align-center ga-2">
                ...
                <span class="text-subtitle-2 font-weight-bold">ห้องปฏิบัติการที่ส่งทดสอบ (Testing Laboratory)</span>
              </div>
              <v-card-text class="pa-4">
                <v-row dense>
                  <v-col cols="12" md="6">
                    <div class="info-label">ชื่อห้องปฏิบัติการ</div>
                    <div class="info-value font-weight-medium">{{ application.laboratory.name }}</div>
                  </v-col>
                  <v-col cols="12" md="6">
                    <div class="info-label">ที่อยู่</div>
                    <div class="info-value">{{ application.laboratory.address }}</div>
                  </v-col>
                </v-row>
              </v-card-text>
            </v-card>
            ...
            <v-card rounded="xl" elevation="0" class="section-card mb-4">
              <div class="section-header px-4 py-3 border-b d-flex align-center ga-2">
                ...
                <span class="text-subtitle-2 font-weight-bold">ประเทศปลายทาง (Country of Destination)</span>
              </div>
              <v-card-text class="pa-4">
            ...
            <v-card rounded="xl" elevation="0" class="section-card mb-4">
              <div class="section-header px-4 py-3 border-b d-flex align-center ga-2">
                ...
                <span class="text-subtitle-2 font-weight-bold">วัตถุประสงค์และปริมาณการส่งออก</span>
              </div>
              <v-card-text class="pa-4">
                <v-row dense>
                  <v-col cols="12" md="6">
                    <div class="info-label">วัตถุประสงค์การสุ่มเก็บตัวอย่าง</div>
                    <div class="info-value">
                      <v-chip size="small" :color="objectiveColor(application.objective)" variant="tonal">
                        {{ objectiveLabelFull(application.objective) }}
                      </v-chip>
                    </div>
                  </v-col>
                  <v-col cols="12" md="6">
                    <div class="info-label">ปริมาณการส่งออก / Export Volume</div>
                    <div class="info-value">
                      {{ application.exportVolume.amount }} {{ application.exportVolume.unit }}
                    </div>
                  </v-col>
                  ...
                  <template v-if="application.objective === 'whole'">
                    <v-col cols="12">
                      ...
                      <div class="text-subtitle-2 font-weight-medium mb-2">ข้อมูลทะเบียนประวัติ (Whole Product System)</div>
                    </v-col>
                    <v-col cols="12" md="4">
                      <div class="info-label">หมายเลขทะเบียนโรงงาน</div>
                      <div class="info-value text-hcex-staff font-weight-bold">{{ application.wholeSystem.factoryRegNo }}</div>
                    </v-col>
                    <v-col cols="12" md="4">
                      <div class="info-label">มาตรฐาน GMP/HACCP</div>
                      <div class="info-value">{{ application.wholeSystem.standard }}</div>
                    </v-col>
                    <v-col cols="12" md="4">
                      <div class="info-label">วันหมดอายุ</div>
                      <div class="info-value">{{ application.wholeSystem.expiryDate }}</div>
                    </v-col>
                    <v-col cols="12">
                      <div class="info-label">วัตถุประสงค์การทดสอบ</div>
                      <div class="info-value">{{ application.wholeSystem.testPurpose }}</div>
                    </v-col>
                  </template>
                </v-row>
              </v-card-text>
            ...
            <v-card rounded="xl" elevation="0" class="section-card mb-4">
              <div class="section-header px-4 py-3 border-b d-flex align-center ga-2">
                ...
                <span class="text-subtitle-2 font-weight-bold">รายการทดสอบตามมาตรฐาน</span>
              </div>
              <v-table density="compact" class="pa-2">
                ...
                <tbody>
                  <tr v-for="(t, idx) in application.testItems" :key="t.name">
                    <td class="text-body-2 text-medium-emphasis">{{ idx + 1 }}</td>
                    <td class="text-body-2 font-weight-medium">{{ t.name }}</td>
                    <td class="text-body-2 text-medium-emphasis">{{ t.criteria }}</td>
                  </tr>
                </tbody>
              </v-table>
              <div class="px-4 py-3 border-t">
                <div class="info-label mb-1">มาตรฐานอ้างอิง (กรมวิชาการเกษตร)</div>
            ...
            >
              <div class="section-header px-4 py-3 border-b d-flex align-center ga-2">
                <v-icon icon="fas fa-circle-info" color="hcex-staff" size="15" />
                <span class="text-subtitle-2 font-weight-bold">ข้อมูลอื่นๆ (Other Data)</span>
              </div>
              <v-card-text class="pa-4">
              ...
              <v-card-text class="pa-4">
                ...
                >
                  <div class="d-flex align-center ga-2">
                    <v-icon
                      :icon="doc.type === 'pdf' ? 'fas fa-file-pdf' : 'fas fa-file-image'"
                  ...
                  <v-radio value="pass" class="mr-6">
                    <template #label>
                      <div class="d-flex align-center ga-2">
                        ...
                        <span class="font-weight-medium">ผ่านการพิจารณา</span>
                      </div>
                    </template>
                  </v-radio>
                  ...
                  <v-radio value="fail">
                    <template #label>
                      <div class="d-flex align-center ga-2">
                        ...
                        <span class="font-weight-medium">ไม่ผ่านการพิจารณา</span>
                      </div>
                    </template>
                  </v-radio>
                </v-radio-group>
    ...
    <v-dialog v-model="sendBackDialog" max-width="400">
      <v-card rounded="xl">
        <v-card-text class="pa-7 text-center">
          ...
          <p class="text-body-2 text-medium-emphasis">ยืนยันการส่งคำขอนี้กลับให้ผู้ประกอบการแก้ไข</p>
        </v-card-text>
        <v-card-actions class="px-6 pb-5">
          <v-row no-gutters class="ga-2 w-100">
            ...
            <v-col>
              <v-btn color="warning" rounded="lg" block @click="sendBackDialog = false">ยืนยัน</v-btn>
            </v-col>
          </v-row>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="rejectDialog" max-width="400">
      <v-card rounded="xl">
        <v-card-text class="pa-7 text-center">
          ...
          <p class="text-body-2 text-medium-emphasis">ยืนยันการปฏิเสธคำขอผล Lab นี้</p>
        </v-card-text>
        <v-card-actions class="px-6 pb-5">
          <v-row no-gutters class="ga-2 w-100">
            <v-col>
              <v-btn variant="tonal" color="grey" rounded="lg" block @click="rejectDialog = false">ยกเลิก</v-btn>
            </v-col>
            <v-col>
              <v-btn color="error" rounded="lg" block @click="rejectDialog = false">ยืนยัน</v-btn>
            </v-col>
          </v-row>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="approveDialog" max-width="400">
      <v-card rounded="xl">
        <v-card-text class="pa-7 text-center">
          ...
          <h3 class="text-h6 font-weight-bold mb-2">ผ่านการพิจารณา</h3>
          <p class="text-body-2 text-medium-emphasis">
            ยืนยันผล Lab ผ่านการพิจารณา ผู้ประกอบการสามารถนำไปใช้ประกอบการออกใบรับรองสุขอนามัยได้
          </p>
        </v-card-text>
        <v-card-actions class="px-6 pb-5">
          <v-row no-gutters class="ga-2 w-100">
            <v-col>
              <v-btn variant="tonal" color="grey" rounded="lg" block @click="approveDialog = false">ยกเลิก</v-btn>
            </v-col>
            <v-col>
              <v-btn color="hcex-staff" rounded="lg" block @click="approveDialog = false">ยืนยัน</v-btn>
            </v-col>
          </v-row>
        </v-card-actions>
      </v-card>
...
const deadlineDays = computed(() => {
  ...
  return Math.ceil((new Date(reviewForm.value.deadline) - new Date()) / (1000 * 60 * 60 * 24));
});

const timelineSteps = [
  { value: 0, title: "ยื่นคำขอ" },
  { value: 1, title: "ตรวจสอบ" },
  { value: 2, title: "ผ่านการพิจารณา" },
];
...
const application = ref({
  ...
  collectingLocation: "โกดังสินค้า บริษัท ไทยฟู้ดส์ แปรรูป จำกัด 456 ถนนรังสิต-นครนายก ต.รังสิต อ.ธัญบุรี จ.ปทุมธานี",
  laboratory: {
  ...
  doareference: "ภาคผนวก 3 ลำดับที่ 12 — มาตรฐานสำหรับสินค้าแปรรูปพืชส่งออกไปญี่ปุ่น เกาหลี และจีน",
...
function objectiveLabel(o) {
  return { hc: "Issuance of HC", lot: "Lot by Lot", whole: "Whole Product System" }[o] ?? o;
}
...
function statusColor(s) {
  return (
    { pending: "info", reviewing: "info", need_edit: "warning", approved: "success", rejected: "error" }[s] ?? "grey"
  );
}
...
function eventIcon(type) {
  return (
    { submit: "fas fa-paper-plane", checking: "fas fa-magnifying-glass", approve: "fas fa-check", improve: "fas fa-rotate-left", reject: "fas fa-xmark" }[type] ?? "fas fa-circle"
  );
}

function eventColor(type) {
  return (
    { submit: "info", checking: "info", approve: "success", improve: "warning", reject: "error" }[type] ?? "grey"
  );
}

function eventLabel(type) {
  return (
    { submit: "ยื่นคำขอ", checking: "ตรวจสอบ", approve: "อนุมัติ", improve: "ส่งกลับ", reject: "ปฏิเสธ" }[type] ?? type
  );
}
</script>
...
.step-circle--done { background: rgb(var(--v-theme-hcex-staff)); color: white; }
.step-circle--active {
...
.step-line { height: 2px; background: rgba(var(--v-border-color), 0.2); margin: 0 4px; margin-bottom: 20px; }
.step-line--done { background: rgb(var(--v-theme-hcex-staff)); }
.section-card { border: 1px solid rgba(var(--v-border-color), var(--v-border-opacity)); }
.section-header { background: rgba(var(--v-theme-surface-variant), 0.4); }
.info-label {
  font-size: 11px; color: rgba(var(--v-theme-on-surface), var(--v-medium-emphasis-opacity));
  margin-bottom: 2px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px;
}
.info-value { font-size: 13px; color: rgb(var(--v-theme-on-surface)); min-height: 20px; }
.item-row {
...
.activity-dot-wrap { display: flex; flex-direction: column; align-items: center; min-width: 22px; }
.activity-dot { width: 22px; height: 22px; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
.activity-line { width: 2px; flex-grow: 1; background: rgba(var(--v-border-color), 0.3); margin-top: 4px; }
