      ...
      <v-col cols="12" md="8">
        <v-tabs v-model="activeTab" color="hcex-staff" class="mb-4">
          <v-tab value="info" prepend-icon="fas fa-file-lines"
            >ข้อมูลคำขอ</v-tab
          >
          <v-tab value="review" prepend-icon="fas fa-scale-balanced">
            บันทึกผลการพิจารณา
          </v-tab>
        </v-tabs>
              ...
              <v-card-text class="pa-5">
                ...
                <template v-if="reviewResult.result === 'approve'">
                  ...
                  <div class="text-caption text-medium-emphasis mb-2">
                    ...
                    สร้างเลขทะเบียน 1 เลข ต่อ 1 ขอบเขตประเทศ —
                    สามารถแก้ไขได้ก่อนยืนยัน
                  </div>
                ...
                <div class="field-label mb-2 mt-3">
                  <div>ผู้ลงนาม</div>
                  <div class="field-label-en">Signatories</div>
                </div>
                <div class="text-caption text-medium-emphasis mb-2">
                  <v-icon icon="fas fa-circle-info" size="11" class="mr-1" />
                  เลือกผู้ลงนามอย่างน้อย 2 คน และไม่เกิน 3 คน
                </div>
                <v-autocomplete
                  ...
                  density="compact"
                  rounded="lg"
                  placeholder="ค้นหาและเลือกผู้ลงนาม..."
                  :rules="[signerRule]"
                  :error-messages="signerErrorMsg"
                  class="mb-4"
                ...
                >
                  <template #chip="{ props, item }">
                    <v-chip
                      ...
                      variant="tonal"
                      size="small"
                      :prepend-icon="item.raw.icon || 'fas fa-user-tie'"
                    >
                      {{ item.raw.name }}
                    </v-chip>
                  </template>
                  <template #item="{ props, item }">
                    <v-list-item
                      ...
                      :prepend-icon="item.raw.icon || 'fas fa-user-tie'"
                      :disabled="
                        !selectedSigners.includes(item.raw.id) &&
                ...
                <v-row class="ga-2" no-gutters>
                  <v-col v-if="reviewResult.result === 'approve'">
                    <v-btn
                      ...
                      :disabled="selectedSigners.length < 2"
...
const reviewResult = reactive({ result: "approve", remark: "" });
...
function submitApprove() {
  ...
  successMessage.value = "อนุมัติคำขอและส่งลงนามเรียบร้อยแล้ว";
...
const signerOptions = [
  {
    id: 1,
    name: "นายอนันต์ วิชาการ",
    position: "ผู้อำนวยการกลุ่ม",
    icon: "fas fa-user-tie",
  },
  {
    id: 2,
    name: "น.ส.วรรณา จันทร์ดี",
    position: "นักวิชาการเกษตรชำนาญการ",
    icon: "fas fa-user-tie",
  },
  {
    id: 3,
    name: "นายสุรชัย พิทักษ์",
    position: "นักวิชาการเกษตรปฏิบัติการ",
    icon: "fas fa-user-tie",
  },
  {
    id: 4,
    name: "น.ส.ปิยะมาศ สุวรรณ",
    position: "นักวิชาการเกษตรชำนาญการพิเศษ",
...
const signerRule = (v) => {
  if (v.length < 2) return "กรุณาเลือกผู้ลงนามอย่างน้อย 2 คน";
  if (v.length > 3) return "เลือกผู้ลงนามได้ไม่เกิน 3 คน";
  return true;
};

const signerErrorMsg = computed(() => {
  if (selectedSigners.value.length === 0) return "";
  if (selectedSigners.value.length < 2)
    return "กรุณาเลือกผู้ลงนามอย่างน้อย 2 คน";
  return "";
});
...
function previewLabSummaryPdf() {
  const rows = labTypes
    .map((lab) => {
      const d = labResultsData[lab.key];
      if (!d) return "";
      const itemRows = Object.entries(d.items)
        .map(
          ([name, v]) => `
          <tr>
            <td class="sub">${name}</td>
            <td>${v.value} <span style="color:#888">${v.unit}</span></td>
            <td class="${v.pass ? "pass" : "fail"}">${v.pass ? "ผ่าน" : "ไม่ผ่าน"}</td>
          </tr>`,
        )
        .join("");
      return `
        <tr class="group-header">
          <td colspan="3">
            ...
            <span class="report-no">${d.reportNo}</span>
            <span class="badge ${d.status === "pass" ? "pass" : "fail"}">${d.status === "pass" ? "ผ่าน" : "ไม่ผ่าน"}</span>
          </td>
        </tr>
        ${itemRows}`;
    })
...
<head>
  <meta charset="UTF-8" />
  <title>ใบสรุปผล LAB — ${application.goods.analysisReportNo}</title>
  <style>
    @page { size: A4; margin: 18mm 20mm; }
    body { font-family: 'Sarabun', 'TH Sarabun New', sans-serif; font-size: 13pt; color: #1a1a1a; }
    h1 { font-size: 17pt; text-align: center; margin: 0 0 4px; }
    .subtitle { text-align: center; font-size: 11pt; color: #555; margin-bottom: 20px; }
    .meta-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 4px 24px; margin-bottom: 20px; border: 1px solid #ddd; border-radius: 6px; padding: 12px 16px; background: #f9fafb; }
    .meta-label { font-size: 10pt; color: #666; }
    .meta-value { font-size: 12pt; font-weight: 600; }
    table { width: 100%; border-collapse: collapse; font-size: 12pt; }
    th { background: #1a3a2a; color: #fff; padding: 8px 12px; text-align: left; }
    td { padding: 6px 12px; border-bottom: 1px solid #e8e8e8; vertical-align: middle; }
    tr.group-header td { background: #f0f4f2; font-size: 12pt; padding: 8px 12px; border-top: 2px solid #2e7d32; }
    tr.sub td { padding-left: 28px; }
    td.pass { color: #2e7d32; font-weight: 600; }
    td.fail { color: #c62828; font-weight: 600; }
    .badge { display: inline-block; padding: 1px 8px; border-radius: 10px; font-size: 10pt; margin-left: 8px; }
    .badge.pass { background: #e8f5e9; color: #2e7d32; }
    .badge.fail { background: #ffebee; color: #c62828; }
    .report-no { font-size: 10pt; color: #666; margin-left: 10px; font-weight: normal; }
    .footer { margin-top: 24px; font-size: 10pt; color: #888; text-align: center; }
  </style>
</head>
<body>
  <h1>ใบสรุปผลการตรวจวิเคราะห์</h1>
  <div class="subtitle">กรมวิชาการเกษตร • Department of Agriculture</div>
  <div class="meta-grid">
    <div><div class="meta-label">เลขรายงานผล</div><div class="meta-value">${application.goods.analysisReportNo}</div></div>
    <div><div class="meta-label">วันที่รายงาน</div><div class="meta-value">${application.goods.analysisDate}</div></div>
    <div><div class="meta-label">เลขคำขอ</div><div class="meta-value">${application.requestNo}</div></div>
    <div><div class="meta-label">ผู้ยื่นคำขอ</div><div class="meta-value">${application.applicantName}</div></div>
    <div style="grid-column:1/-1"><div class="meta-label">ผู้ผลิต</div><div class="meta-value">${application.goods.manufacturer}</div></div>
    <div style="grid-column:1/-1"><div class="meta-label">รายละเอียดสินค้า</div><div class="meta-value" style="font-family:monospace;white-space:pre-wrap;text-transform:uppercase">${application.goods.descriptionOfGoods}</div></div>
  </div>
  <table>
    <thead>
      <tr>
        <th style="width:44%">รายการตรวจ</th>
        <th style="width:30%">ผลที่ตรวจพบ</th>
        <th style="width:26%">สรุปผล</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>
