          ...
          <v-card-text class="pa-5">
            <div class="d-flex align-center justify-space-between mb-3">
              <div
                ...
                :style="`background: rgba(var(--v-theme-${card.color}), 0.1)`"
              >
                <v-icon :icon="card.icon" :color="card.color" size="20" />
              </div>
              <v-chip size="x-small" :color="card.chipColor" variant="tonal">{{
                card.chipLabel
              }}</v-chip>
            </div>
        ...
        <v-card rounded="xl" elevation="0" class="section-card h-100">
          ...
          >
            ...
            <v-btn
              ...
              color="admin"
              append-icon="fas fa-arrow-right"
              @click="goToCertificateExpiration"
              >ดูทั้งหมด</v-btn
            >
          </v-card-title>
          <v-divider />
          <v-list density="compact" class="pa-2">
            ...
            >
              ...
              <v-list-item-subtitle class="text-caption"
                >หมดอายุ: {{ cert.expiry }}</v-list-item-subtitle
              >
              <template #append>
                <v-chip
                  size="x-small"
            ...
            <div v-for="storage in storages" :key="storage.type" class="mb-4">
              <div class="d-flex justify-space-between mb-1">
                ...
                <span class="text-body-2 text-medium-emphasis"
                  >{{ storage.used }} / {{ storage.total }}</span
                >
              </div>
              <v-progress-linear
...
const summaryCards = [
  ...
  {
    label: "Certificates (หมดเร็ว ๆ นี้)",
    value: "1",
