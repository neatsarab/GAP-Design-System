      ...
      <v-col v-for="storage in storages" :key="storage.type" cols="12" md="6">
        <v-card rounded="xl" elevation="0" class="section-card pa-5">
          <div class="d-flex align-center ga-3 mb-4">
            <div
              class="storage-icon"
              :style="`background: rgba(var(--v-theme-${usageColor(storage.percent)}), 0.1)`"
            >
              <v-icon
                icon="fas fa-hard-drive"
                :color="usageColor(storage.percent)"
                size="22"
              />
            </div>
            <div>
              ...
              <div class="text-caption text-medium-emphasis">
                {{ storage.used }} ใช้งาน จาก {{ storage.total }}
              </div>
            </div>
            <v-spacer />
            <v-chip
              v-if="storage.percent >= 80"
              size="small"
              color="error"
              variant="tonal"
          ...
          <v-row dense class="mt-2">
            <v-col cols="4" class="text-center">
              <div
                ...
                :class="`text-${usageColor(storage.percent)}`"
              >
                {{ storage.percent }}%
              </div>
        ...
        <template #item.percent="{ item }">
          <div class="d-flex align-center ga-2" style="min-width: 160px">
            ...
            <span
              ...
              :class="`text-${usageColor(item.percent)}`"
              >{{ item.percent }}%</span
            >
          </div>
        </template>
