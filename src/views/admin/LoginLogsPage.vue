<template>
  <div>
    <div class="d-flex align-center justify-space-between mb-6">
      ...
      <v-btn color="admin" rounded="lg" prepend-icon="fas fa-file-export"
        >Export Log</v-btn
      >
    </div>
      ...
      <v-card-text class="pa-4">
        <v-row dense align="center">
          ...
          <v-col cols="12" sm="3" md="2">
            <div class="field-label mb-1">
              <div>วันที่</div>
              <div class="field-label-en">Date</div>
            </div>
            <v-text-field
              v-model="filterDate"
              type="date"
    ...
    <v-card rounded="xl" elevation="0" class="data-card">
      ...
      >
        <template #item.role="{ item }">
          <v-chip
            size="x-small"
            :color="item.role === 'Administrator' ? 'admin' : 'default'"
            variant="tonal"
            >{{ item.role }}</v-chip
          >
        </template>
...
const filteredLogs = computed(() => {
  return logs.filter((l) => {
    if (filterStatus.value !== "ทั้งหมด" && l.status !== filterStatus.value)
      return false;
    return true;
  });
});
