    ...
    <v-card rounded="xl" elevation="0" class="data-card">
      <v-data-table :headers="headers" :items="apis" rounded="xl" hover>
        <template #item.status="{ item }">
          <v-chip :color="apiStatusColor(item.status)" size="small" variant="tonal" :prepend-icon="apiStatusIcon(item.status)">
            {{ item.status }}
          </v-chip>
        </template>
        <template #item.responseTime="{ item }">
          <span class="text-body-2" :class="item.responseTime > 200 ? 'text-warning' : 'text-success'">
            {{ item.status === 'Connected' ? item.responseTime + ' ms' : '—' }}
          </span>
        </template>
        <template #item.actions="{ item }">
          <v-btn size="x-small" color="admin" variant="tonal" rounded="lg" prepend-icon="fas fa-satellite-dish" @click="pingApi(item)">Ping</v-btn>
        </template>
      </v-data-table>
    </v-card>
  </div>
