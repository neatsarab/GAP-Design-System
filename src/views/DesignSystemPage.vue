<template>
  <div>
    ...
    <!-- ════════════════════════════════
         SIDEBAR NAV
    ════════════════════════════════ -->
    <v-navigation-drawer v-model="drawer" :rail="rail" permanent color="surface" class="ds-sidebar">
      ...
      <v-list-item nav class="py-4 px-4">
        ...
        <template v-slot:append>
          <v-btn
            ...
            variant="text" color="on-surface-variant" size="small"
            @click="rail = !rail"
          />
        </template>
      </v-list-item>
      ...
      <v-list density="compact" nav class="mt-1 px-2">
        ...
        <v-list-item
          v-for="s in navSections.slice(0, 2)" :key="s.id"
          :prepend-icon="s.icon" :title="s.title" :value="s.id"
        ...
        <v-list-item
          v-for="s in navSections.slice(2, 9)" :key="s.id"
          :prepend-icon="s.icon" :title="s.title" :value="s.id"
          :active="activeSection === s.id"
          active-color="primary"
          rounded="lg" class="mb-1"
        ...
        <v-list-item
          v-for="s in navSections.slice(9)" :key="s.id"
          :prepend-icon="s.icon" :title="s.title" :value="s.id"
          :active="activeSection === s.id"
          active-color="primary"
          rounded="lg" class="mb-1"
    ...
    <v-app-bar flat height="70" class="ds-app-bar">
      ...
      <div class="d-none d-sm-block ml-3" style="width:210px">
        <v-text-field
          placeholder="ค้นหา..." prepend-inner-icon="fas fa-magnifying-glass"
          variant="solo-filled" density="compact" hide-details flat
          rounded="lg" bg-color="surface-variant"
        />
      </div>
      ...
      <div class="d-flex align-center ga-1 mr-3">
        ...
        <v-chip variant="outlined" color="primary" class="user-chip mr-3" prepend-icon="fas fa-user">
          นิธิพร เทิบจันทึก
    ...
    <v-main class="bg-background">
      <v-container fluid class="pa-5 pa-md-7" style="max-width: 1280px">
                ...
                <div class="d-flex align-start">
                  <div class="flex-grow-1">
                    ...
                    <div class="d-flex align-center ga-1">
                      <v-chip :color="card.trend >= 0 ? 'success' : 'error'" size="x-small" variant="tonal" class="px-1">
                  ...
                  <div class="stat-icon-box ml-3" :style="`background:rgba(var(--v-theme-${card.color}),0.12)`">
        ...
        <section :id="navSections[0].id" class="mb-10">
          <DsSectionTitle :icon="navSections[0].icon" :title="navSections[0].title" />
          <DsColors :is-dark="isDark" />
        </section>

        <!-- 2. TYPOGRAPHY -->
        <section :id="navSections[1].id" class="mb-10">
          <DsSectionTitle :icon="navSections[1].icon" :title="navSections[1].title" />
          <DsTypography />
        </section>

        <!-- 3. BUTTONS -->
        <section :id="navSections[2].id" class="mb-10">
          <DsSectionTitle :icon="navSections[2].icon" :title="navSections[2].title" />
          <DsButtons />
        </section>

        <!-- 4. STATUS CHIPS -->
        <section :id="navSections[3].id" class="mb-10">
          <DsSectionTitle :icon="navSections[3].icon" :title="navSections[3].title" />
          <DsChips />
        </section>

        <!-- 5. FORM INPUTS -->
        <section :id="navSections[4].id" class="mb-10">
          <DsSectionTitle :icon="navSections[4].icon" :title="navSections[4].title" />
          <DsForms />
        </section>

        <!-- 6. CARDS -->
        <section :id="navSections[5].id" class="mb-10">
          <DsSectionTitle :icon="navSections[5].icon" :title="navSections[5].title" />
          <DsCards />
        </section>

        <!-- 7. DATA TABLE -->
        <section :id="navSections[6].id" class="mb-10">
          <DsSectionTitle :icon="navSections[6].icon" :title="navSections[6].title" />
          <DsTable />
        </section>

        <!-- 8. ALERTS -->
        <section :id="navSections[7].id" class="mb-10">
          <DsSectionTitle :icon="navSections[7].icon" :title="navSections[7].title" />
          <DsAlerts />
        </section>

        <!-- 9. DIALOGS -->
        <section :id="navSections[8].id" class="mb-10">
          <DsSectionTitle :icon="navSections[8].icon" :title="navSections[8].title" />
          <DsDialogs />
        </section>

        <!-- 10. STEPPER -->
        <section :id="navSections[9].id" class="mb-10">
          <DsSectionTitle :icon="navSections[9].icon" :title="navSections[9].title" />
          <DsStepper />
        </section>

        <!-- 11. PANELS & LISTS -->
        <section :id="navSections[10].id" class="mb-10">
          <DsSectionTitle :icon="navSections[10].icon" :title="navSections[10].title" />
          <DsPanels />
        </section>

        <!-- 12. PROGRESS & LOADING -->
        <section :id="navSections[11].id" class="mb-10">
          <DsSectionTitle :icon="navSections[11].icon" :title="navSections[11].title" />
          <DsProgress />
        </section>

        <!-- 13. TIMELINE -->
        <section :id="navSections[12].id" class="mb-10">
          <DsSectionTitle :icon="navSections[12].icon" :title="navSections[12].title" />
          <DsTimeline />
        </section>

        <!-- 14. NAVIGATION -->
        <section :id="navSections[13].id" class="mb-10">
          <DsSectionTitle :icon="navSections[13].icon" :title="navSections[13].title" />
          <DsNavigation />
        </section>

        <!-- 15. ICONS -->
        <section :id="navSections[14].id" class="mb-10">
          <DsSectionTitle :icon="navSections[14].icon" :title="navSections[14].title" />
          <DsIcons />
        </section>

        <!-- 16. AppStatusChip -->
        <section :id="navSections[15].id" class="mb-10">
          <DsSectionTitle :icon="navSections[15].icon" :title="navSections[15].title" />
          <DsStatusChip />
        </section>
...
const navSections = [
  ...
  { id: 'status-chip', icon: 'fas fa-circle-half-stroke', title: '16. AppStatusChip' },
]
