      ...
      <v-card-text class="pa-4">
        <v-row dense align="center">
          ...
          <v-col cols="12" sm="6" md="6">
            ...
            <v-autocomplete
              v-model="filters.packingHouseName"
              :items="packingHouseNameOptions"
              item-title="label"
              item-value="value"
              placeholder="ทั้งหมด"
              variant="outlined"
...
const packingHouseNameOptions = [
  {
    label: "บริษัท เอส.ดับบลิว เทค แอนด์ มีเดีย จำกัด",
    value: "บริษัท เอส.ดับบลิว เทค แอนด์ มีเดีย จำกัด",
  },
];
...
const filteredItems = computed(() => {
  ...
  if (filters.packingHouseName)
    items = items.filter(
      (i) => i.packingHouseName === filters.packingHouseName,
    );
