<template>
  <div>
    <div class="d-flex align-center justify-space-between mb-6 flex-wrap ga-3">
      <div>
        <h1 class="text-h5 font-weight-bold mb-1">แดชบอร์ด DOA โรงงานผลิตสินค้าพืช</h1>
        <p class="text-body-2 text-medium-emphasis mb-0">ภาพรวมคำขอขึ้นทะเบียนโรงงานของคุณ</p>
      </div>
      <v-btn color="doa-user" prepend-icon="fas fa-file-pen" rounded="lg" @click="router.push('/doa/user/applications/new')">
        ยื่นคำขอใหม่
      </v-btn>
    </div>
      ...
      <v-col cols="12" md="8">
        <v-card rounded="xl" elevation="0" class="h-100">
          ...
          <v-list lines="two" class="pa-2">
            <v-list-item v-for="app in recentApplications" :key="app.id" rounded="lg" class="mb-1" @click="router.push(`/doa/user/applications/${app.id}`)">
              <template #prepend>
      ...
      <v-col cols="12" md="4">
        ...
        <v-card rounded="xl" elevation="0">
          ...
          <v-list density="compact" nav class="px-2 pb-3">
            <v-list-item prepend-icon="fas fa-file-pen" title="ยื่นคำขอ DOA ใหม่" rounded="lg" color="doa-user" class="mb-1" @click="router.push('/doa/user/applications/new')" />
            <v-list-item prepend-icon="fas fa-file-lines" title="รายการคำขอ" rounded="lg" color="doa-user" @click="router.push('/doa/user/applications')" />
          </v-list>
        </v-card>
      </v-col>
    </v-row>
...
const stats = [
  { label: "คำขอทั้งหมด", value: 2, icon: "fas fa-file-lines", color: "doa-user" },
  { label: "อยู่ระหว่างตรวจสอบ", value: 1, icon: "fas fa-magnifying-glass", color: "doa-user" },
  { label: "อนุมัติแล้ว", value: 1, icon: "fas fa-circle-check", color: "success" },
  { label: "โรงงานที่ขึ้นทะเบียน", value: 1, icon: "fas fa-industry", color: "info" },
];
