        ...
        <v-card rounded="xl" elevation="0" class="data-card">
            <v-data-table v-model="selected" :headers="headers" :items="filteredItems" show-select item-value="id"
                ...
                <template #item.reviewResult="{ item }">
                    <v-chip :color="item.reviewResult === 'approved' ? 'success' : 'warning'" size="small"
                        variant="tonal">
                        {{ item.reviewResult === "approved" ? "ผ่าน" : "ปรับปรุง" }}
                    </v-chip>
                </template>
                <template #item.actions="{ item }">
                    <v-btn size="small" color="doa-staff" variant="tonal" rounded="lg" prepend-icon="fas fa-pen-nib"
                        @click.stop="goToSigning(item.id)">
                        ลงนาม
                    </v-btn>
                </template>
            </v-data-table>
...
function goToSigning(id) {
    router.push({ name: "DOAStaffApplicationDetail", params: { id }, query: { step: 3 } });
}
const selected = ref([]);
