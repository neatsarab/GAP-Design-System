<template>
    <div style="--v-theme-primary: var(--v-theme-doa-staff)">
                                ...
                                <v-row dense>
                                    ...
                                    <v-col cols="6" md="4">
                                        ...
                                        <div class="info-value">
                                            <v-chip size="x-small" color="doa-staff" variant="tonal">{{
                        ...
                        <v-card rounded="xl" elevation="0" class="section-card mb-4">
                            ...
                            <v-card-text class="pa-4 pb-3">
                                ...
                                <div class="d-flex flex-wrap ga-2">
                                    <v-chip v-for="c in application.countries" :key="c" size="small" variant="tonal"
                                                ...
                                                <v-tooltip text="ดูข้อมูล" location="top">
                                                    <template #activator="{ props }">
                                                        <v-btn v-bind="props" icon size="x-small" variant="text"
                                                            color="doa-staff" @click="openFactoryDialog(factory)">
                                        ...
                                        <td>
                                            <div class="d-flex ga-1 justify-end">
                                                <v-tooltip text="ดูข้อมูล" location="top">
                                                    <template #activator="{ props }">
                                                        <v-btn v-bind="props" icon size="x-small" variant="text"
                                                            color="doa-staff" @click="openGapDialog(gap)">
                                                            <v-icon icon="fas fa-eye" size="14" />
                                                        </v-btn>
                                                    </template>
                                                </v-tooltip>
                                        ...
                                        <v-table density="compact">
                                            ...
                                            <tbody>
                                                <tr v-for="(item, idx) in certNumbers" :key="idx">
                                                    <td>
                                                        <v-chip size="small" color="doa-staff" variant="tonal">{{
                                                            item.country }}</v-chip>
                                                    </td>
                                                    <td class="py-1">
                                                        <v-text-field v-model="item.certNo" variant="outlined"
                                                            density="compact" rounded="lg" hide-details
                                                            style="min-width: 180px; max-width: 260px" />
                                                    </td>
                                                    <td>
                                                        <v-btn size="small" variant="tonal" color="doa-staff"
                                                            rounded="lg" prepend-icon="fas fa-eye"
                                ...
                                <v-row class="ga-2" no-gutters>
                                    <v-col v-if="reviewResult.result === 'approve'">
                                        <v-btn color="doa-staff" variant="flat" block rounded="lg"
                                            prepend-icon="fas fa-pen-nib" @click="approveDialog = true">
                                            ส่งลงนาม
                                        </v-btn>
                                    </v-col>
                                    <v-col v-if="reviewResult.result === 'reject'">
                                        <v-btn color="error" variant="tonal" block rounded="lg"
                                            prepend-icon="fas fa-circle-xmark" @click="rejectDialog = true">
                                            ไม่อนุมัติ
                                        </v-btn>
                                    </v-col>
                                </v-row>
                    ...
                    <v-card rounded="xl" elevation="0" class="section-card">
                        ...
                        <v-card-text class="pa-4">
                            <div class="activity-timeline">
                                <div v-for="(event, i) in application.activityLog" :key="i" class="activity-item">
                                    ...
                                    <div class="activity-content pb-4">
                                        ...
                                        <v-btn v-if="event.type !== 'submit'" size="x-small" variant="text"
                                            color="doa-staff" class="mt-1 px-0" @click="openActivityDetail(event)">
                                            ดูรายละเอียด
        ...
        <v-dialog v-model="factoryDialog" max-width="480">
            <v-card rounded="xl">
                <v-card-title class="d-flex align-center ga-2 pa-5 pb-3">
                    ...
                    <v-spacer />
                    <v-btn icon="fas fa-xmark" variant="text" size="small" @click="factoryDialog = false" />
                </v-card-title>
                <v-divider />
                <v-card-text class="pa-5">
                ...
                <v-card-actions class="px-5 pb-5 pt-3">
                    <v-btn color="doa-staff" variant="tonal" prepend-icon="fas fa-download" rounded="lg" block>
                        ดาวน์โหลด
                    </v-btn>
                </v-card-actions>
            </v-card>
        ...
        <v-dialog v-model="gapDialog" max-width="480">
            <v-card rounded="xl">
                <v-card-title class="d-flex align-center ga-2 pa-5 pb-3">
                    ...
                    <v-spacer />
                    <v-btn icon="fas fa-xmark" variant="text" size="small" @click="gapDialog = false" />
                </v-card-title>
                <v-divider />
                <v-card-text class="pa-5">
                    ...
                    <v-row no-gutters class="ga-2 w-100">
                        <v-col>
                            <v-btn variant="tonal" color="grey" rounded="lg" block
                                @click="approveDialog = false">ยกเลิก</v-btn>
                        </v-col>
                        <v-col>
                            <v-btn color="doa-staff" rounded="lg" block @click="submitApprove">ยืนยัน</v-btn>
                        </v-col>
                    </v-row>
                </v-card-actions>
            </v-card>
        ...
        <v-dialog v-model="rejectDialog" max-width="400">
            <v-card rounded="xl">
                <v-card-text class="pa-7 text-center">
                    <div class="confirm-ring mx-auto mb-4" style="background: rgba(var(--v-theme-error), 0.1)">
                ...
                <v-card-actions class="px-6 pb-5">
                    <v-row no-gutters class="ga-2 w-100">
                        <v-col>
                            <v-btn variant="tonal" color="grey" rounded="lg" block
                                @click="rejectDialog = false">ยกเลิก</v-btn>
                        </v-col>
                        <v-col>
                            <v-btn color="error" rounded="lg" block @click="submitReject">ยืนยัน</v-btn>
                        </v-col>
                    </v-row>
                </v-card-actions>
            </v-card>
        ...
        <v-dialog v-model="activityDetailDialog" max-width="420">
            <v-card rounded="xl">
                ...
                <v-card-actions class="px-6 pb-5 pt-0">
                    <v-btn color="doa-staff" variant="tonal" rounded="lg" block
