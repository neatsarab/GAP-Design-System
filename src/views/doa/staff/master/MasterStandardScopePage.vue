<template>
    <div>
            ...
            <v-card-text class="pa-4">
                <v-row dense align="center">
                    <v-col cols="12" sm="5" md="4">
                        ...
                        <v-text-field v-model="search" variant="outlined" density="compact" rounded="lg" hide-details
                            placeholder="ค้นหารหัส / ชื่อมาตรฐาน..." prepend-inner-icon="fas fa-magnifying-glass"
                            clearable />
                    </v-col>
                    <v-col cols="12" sm="4" md="4">
                        <div class="field-label">
                            <div>หมวดหมู่มาตรฐาน</div>
                            <div class="field-label-en">Status</div>
                        </div>
                        <v-autocomplete v-model="filterCategory" :items="categoryOptions" item-title="label"
                            item-value="value" placeholder="หมวดหมู่ทั้งหมด" variant="outlined" density="compact"
                    ...
                    <v-col cols="12" sm="3" md="4">
                        ...
                        <v-autocomplete v-model="filterStatus" :items="statusOptions" item-title="label"
                            item-value="value" placeholder="สถานะทั้งหมด" variant="outlined" density="compact"
                            rounded="lg" hide-details clearable />
                    </v-col>
                </v-row>
                <v-row dense>
        ...
        <v-card rounded="xl" elevation="0" class="data-card">
            <v-data-table :headers="headers" :items="filteredItems" :search="search" rounded="xl" hover>
                ...
                <template #item.status="{ item }">
                    <v-chip size="small" variant="tonal" :color="item.status === 'active' ? 'success' : 'default'">
                ...
                <template #item.actions="{ item }">
                    <div class="d-flex ga-1 justify-end">
                        <v-btn size="x-small" variant="tonal" color="doa-staff" rounded="lg" icon="fas fa-pen"
                            @click="openEdit(item)" />
                        <v-btn size="x-small" variant="tonal" color="error" rounded="lg" icon="fas fa-trash"
            ...
            <v-card rounded="xl">
                ...
                <v-card-text class="pa-6">
                    <v-row dense>
                        <v-col cols="12" sm="4">
                            <div class="field-label">
                                <div>รหัสมาตรฐาน <span class="req">*</span></div>
                                <div class="field-label-en">Standard Code</div>
                            </div>
                            <v-text-field v-model="form.code" variant="outlined" density="compact" rounded="lg"
                                hide-details placeholder="เช่น มกษ.9023" />
                        </v-col>
                        <v-col cols="12" sm="8">
                            <div class="field-label">
                                <div>หมวดหมู่ <span class="req">*</span></div>
                                <div class="field-label-en">Category</div>
                            </div>
                            <v-autocomplete v-model="form.category" :items="categoryOptions" item-title="label"
                                item-value="value" variant="outlined" density="compact" rounded="lg" hide-details />
                        </v-col>
                        <v-col cols="12">
                            <div class="field-label mt-2">
                                <div>ชื่อมาตรฐาน (ภาษาไทย) <span class="req">*</span></div>
                                <div class="field-label-en">Standard Name (Thai)</div>
                            </div>
                            <v-text-field v-model="form.nameTh" variant="outlined" density="compact" rounded="lg"
                                hide-details placeholder="ชื่อเต็มภาษาไทย" />
                        </v-col>
                        <v-col cols="12">
                            <div class="field-label mt-2">
                                <div>Standard Name (English)</div>
                                <div class="field-label-en">Standard Name (English)</div>
                            </div>
                            <v-text-field v-model="form.nameEn" variant="outlined" density="compact" rounded="lg"
                                hide-details placeholder="Full name in English" />
                        </v-col>
                        <v-col cols="12">
                            <div class="field-label mt-2">
                                <div>คำอธิบาย</div>
                            ...
                            <v-textarea v-model="form.description" variant="outlined" density="compact" rounded="lg"
                                hide-details rows="2" placeholder="รายละเอียดเพิ่มเติม..." />
                        </v-col>
                        <v-col cols="12">
                            <div class="field-label mt-2">
                                <div>สถานะ</div>
                            ...
                            <v-switch v-model="form.isActive" color="doa-staff"
                                :label="form.isActive ? 'ใช้งาน' : 'ปิดใช้งาน'" hide-details density="compact" />
                        </v-col>
                    </v-row>
                </v-card-text>
                <v-divider />
                <v-card-actions class="pa-5 ga-2">
                    ...
                    <v-btn variant="flat" color="doa-staff" rounded="lg" @click="saveForm">{{ editingItem ? 'บันทึกการแก้ไข' :
                        'เพิ่มมาตรฐาน'
                    }}</v-btn>
                </v-card-actions>
            </v-card>
            ...
            <v-card rounded="xl">
                <v-card-text class="pa-7 text-center">
                    ...
                    <p class="text-body-2 text-medium-emphasis mb-0">
                        ต้องการลบ <strong>{{ deletingItem?.code }}</strong>
...
const headers = [
    ...
    { title: "รหัสมาตรฐาน", key: "code", sortable: true, width: "15%" },
    { title: "ชื่อขอบข่ายมาตรฐาน", key: "nameTh", sortable: true },
