<template>
  <div>
      ...
      <v-col v-for="type in appTypes" :key="type.route" cols="12" md="4">
        ...
        >
          <v-card-text class="pa-7 d-flex flex-column align-center text-center">
            ...
            <!-- Features list -->
            <v-list density="compact" class="w-100 text-left mb-5 bg-transparent pa-0">
              <v-list-item v-for="f in type.features" :key="f"
    ...
    <!-- Info Alert -->
    <v-alert type="info" variant="tonal" class="mt-6" prepend-icon="fas fa-circle-info">
      <strong>หมายเหตุ:</strong> ใบคำขอทุกประเภทต้องใช้เอกสารสำเนาโฉนดที่ดิน และผลวิเคราะห์คุณภาพน้ำ
      สามารถศึกษาคู่มือการยื่นคำขอได้ที่
      <v-btn variant="text" size="x-small" color="info" class="pa-0 ml-1">ดาวน์โหลดคู่มือ</v-btn>
    </v-alert>
  </div>
</template>
...
const appTypes = [
  {
    route: '/app/applications/new/individual',
    title: 'คำขอรายเดี่ยว',
    badge: 'ทั่วไป',
    icon: 'fas fa-user',
    color: 'primary',
    description: 'สำหรับเกษตรกรรายบุคคลที่ต้องการขอรับรองแหล่งผลิต GAP สำหรับแปลงของตนเอง',
    features: [
      'เกษตรกรยื่นคำขอเอง',
  ...
  {
    route: '/app/applications/new/group',
    title: 'คำขอรายกลุ่ม',
    badge: 'กลุ่มเกษตรกร',
    icon: 'fas fa-users',
    color: 'info',
    description: 'สำหรับกลุ่มเกษตรกร วิสาหกิจชุมชน หรือสหกรณ์ที่ต้องการยื่นคำขอพร้อมกัน',
    features: [
  ...
  {
    route: '/app/applications/new/amendment',
    title: 'ขอแก้ไข / ยกเลิกใบรับรอง',
    badge: null,
    icon: 'fas fa-file-pen',
    color: 'warning',
    description: 'สำหรับผู้ที่ต้องการแก้ไขข้อมูลในใบรับรองที่ออกแล้ว หรือขอยกเลิกใบรับรอง',
    features: [
...
.type-card:hover {
  ...
  box-shadow: 0 12px 32px rgba(var(--v-theme-primary), 0.1) !important;
}
.type-icon-box {
  width: 88px;
  height: 88px;
