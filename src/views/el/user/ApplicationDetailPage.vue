        ...
        <div class="d-flex align-center ga-3 mb-6 flex-wrap">
            ...
            <div class="flex-grow-1">
                <div class="d-flex align-center ga-2 mb-1 flex-wrap">
                    ...
                    <v-chip size="small" :color="statusColor(app.status)" variant="tonal"
                            ...
                            <v-table class="checklist-table border">
                                <thead>
                                    <tr class="bg-grey-lighten-3">
                                        <th class="text-center font-weight-bold border" style="width: 10%">รายการเอกสาร
                                        </th>
                                        <th class="text-center font-weight-bold border" style="width: 10%">เอกสารแนบ
                                        </th>
                                        <th class="text-center font-weight-bold border" style="width: 10%">
                                            ผลการตรวจเอกสาร</th>
                                        <th class="text-center font-weight-bold border" style="width: 10%">หมายเหตุ</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="(item, index) in documentList" :key="index">
                                        <td class="pa-4 border align-start">
                                            <div class="text-body-2">{{ index + 1 }}. {{ item.name }}</div>
                                        </td>

                                        <td class="text-center border">
                                            <div v-if="item.file" class="d-flex justify-center align-center">
                                                <v-btn icon="fas fa-file-pdf" variant="text" color="el-user"
                                                    size="large"></v-btn>
                                            </div>
                                            <div v-else class="pa-2">
                                                <v-file-input label="Browse..." density="compact" variant="outlined"
                                                    hide-details prepend-icon="" class="custom-file-input">
                                                    <template #prepend-inner>
                                                        <v-icon icon="fas fa-folder-open" size="small"
                                                            class="mr-2"></v-icon>
                                                    </template>
                                                </v-file-input>
                                            </div>
                                        ...
                                        <td class="text-center border px-2">
                                            <v-select v-model="item.status"
                                                :items="['ผ่าน', 'ไม่ผ่าน', 'ไม่เกี่ยวข้อง']" density="compact"
                                                variant="outlined" hide-details class="status-select"></v-select>
                                        </td>

                                        <td class="text-center border pa-2">
                                            <v-textarea v-model="item.remark" rows="2" density="compact"
                                                variant="outlined" hide-details no-resize
                                                class="remark-textarea"></v-textarea>
                                        </td>
                                    </tr>
                                </tbody>
                            </v-table>
...
const documentList = ref([
    {
        name: 'หนังสือแสดงความประสงค์การเข้าร่วมการจัดทำบัญชี Establishment List ลงนามโดยผู้มีอำนาจของบริษัท',
        file: true, // สมมติว่ามีไฟล์แนบมาแล้ว
        status: 'ผ่าน',
        remark: ''
    },
    {
        name: 'เอกสารการมอบอำนาจให้เจ้าหน้าที่บริษัทดำเนินการจัดทำบัญชี Establishment List ในกรณีที่หนังสือไม่ได้ลงนามโดยผู้มีอำนาจของบริษัท',
        file: false, // ยังไม่มีไฟล์
        status: 'ผ่าน',
        remark: ''
    }
])
