        ...
        <template v-if="currentStep === 0">
          ...
          <v-card rounded="xl" elevation="0" class="mb-5 section-card">
            <v-card-title class="pa-5 pb-0 section-title font-weight-bold"
              >1. ข้อมูลเกษตรกรที่ขอใบรับรอง GAP</v-card-title
            >
            <v-card-text class="pa-5">
              <div class="d-flex align-center ga-3 mb-4 flex-wrap">
                <div style="width: 300px">
                  <v-autocomplete
                    v-model="selectedMasterFarmer"
                    :items="masterFarmers"
                    item-title="name"
                    item-value="name"
                    placeholder="เลือกจากฐานข้อมูลเดิม"
                    variant="outlined"
                    density="compact"
                    hide-details
                    rounded="lg"
                ...
                <v-btn
                  ...
                  rounded="lg"
                  :disabled="!selectedFarmerRows.length"
                  @click="removeItems('farmer')"
                  >ลบ</v-btn
                >
              </div>
              <v-data-table
                v-model="selectedFarmerRows"
              ...
              >
                <template v-slot:item.actions="{ item }"
                  ><v-btn
                    ...
                    size="small"
                    color="warning"
                    @click="goToEditPage(item, 'farmer')"
                /></template>
              </v-data-table>
            </v-card-text>
          </v-card>
          ...
          <v-card rounded="xl" elevation="0" class="mb-5 section-card">
            <v-card-title class="pa-5 pb-0 section-title font-weight-bold"
              >2. ข้อมูลแปลงเกษตรกรที่ขอใบรับรอง GAP</v-card-title
            >
            <v-card-text class="pa-5">
              <div class="d-flex align-center ga-3 mb-4 flex-wrap">
                <v-spacer />
                ...
                <v-btn
                  ...
                  prepend-icon="fas fa-xmark"
                  rounded="lg"
                  :disabled="!selectedFarmRows.length"
                  @click="removeItems('farm')"
                  >ลบ</v-btn
                >
              </div>
              <v-data-table
                v-model="selectedFarmRows"
                :headers="farmHeaders"
                :items="farms"
                show-select
                item-value="id"
          ...
          <v-card rounded="xl" elevation="0" class="mb-5 section-card">
            <v-card-title class="pa-5 pb-0 section-title font-weight-bold"
              >3. ข้อมูลใบรับรอง GMP ของโรงคัดบรรจุ</v-card-title
            >
            <v-card-text class="pa-5">
              <div class="d-flex align-center ga-3 mb-4 flex-wrap">
                <div style="width: 300px">
                  ...
                  <v-autocomplete
                    v-model="selectedMasterFactory"
                    :items="masterFactories"
                    item-title="cropName"
                    item-value="id"
                    placeholder="เลือกข้อมูลโรงคัดบรรจุ"
                    variant="outlined"
                    density="compact"
                    hide-details
                    rounded="lg"
                ...
                <v-btn
                  ...
                  prepend-icon="fas fa-xmark"
                  rounded="lg"
                  :disabled="!selectedFactoryRows.length"
                  @click="removeItems('factory')"
                  >ลบ</v-btn
                >
              </div>

              <v-data-table
                v-model="selectedFactoryRows"
              ...
              >
                <template v-slot:item.actions="{ item }"
                  ><v-btn
                    ...
                    size="small"
                    color="warning"
                    @click="goToEditPage(item, 'factory')"
                /></template>
              </v-data-table>
            </v-card-text>
          </v-card>
        ...
        <v-card rounded="xl" elevation="0" class="mt-6 section-card">
          <v-card-text class="pa-4 d-flex justify-space-between align-center">
            <v-btn
              ...
              >{{ currentStep === 0 ? "ยกเลิก" : "ย้อนกลับ" }}</v-btn
            >
          ...
          <v-card-text class="pa-8">
            <v-row dense v-if="editType === 'farmer'">
              ...
              <v-col cols="12" md="6">
                <div class="field-label">
                  เลขที่ (บัตรประชาชน / นิติบุคคล) <span class="req">*</span>
                </div>
                <v-text-field
                  v-model="tempData.idNo"
                  variant="outlined"
                  rounded="lg"
                  density="comfortable"
              ...
              <v-col cols="12" md="6">
                <div class="field-label mb-1">
                  บ้านเลขที่ <span class="req">*</span>
                </div>
                <v-text-field
                  v-model="tempData.addressNo"
                  variant="outlined"
                  rounded="lg"
                  density="comfortable"
                />
              ...
              <v-col cols="12" md="4">
                ...
                <v-autocomplete
                  ...
                  :items="['กรุงเทพฯ', 'ปทุมธานี', 'สุพรรณบุรี', 'เชียงใหม่']"
                  variant="outlined"
                  rounded="lg"
                  density="comfortable"
                />
              ...
              <v-col cols="12" class="mt-n5">
                <v-card variant="outlined" class="pa-6 rounded-xl">
                  <v-row dense>
                    ...
                    <v-col cols="12" md="6">
                      <div class="field-label mb-1">
                        ชื่อเกษตรกร <span class="req">*</span>
                      </div>
                      <v-autocomplete
                    ...
                    <template v-if="tempData.certType === 'GAP'">
                      <v-col cols="12" md="6">
                        ...
                        <v-text-field
                          ...
                          placeholder="ดึงมาจาก GAP master data ราย item"
                          variant="outlined"
                          rounded="lg"
                          density="comfortable"
                          hide-details
                      ...
                      <v-col cols="12" md="6">
                        <div class="field-label mb-1">
                          รหัสใบรับรอง <span class="req">*</span>
                        </div>
                        <v-text-field
                          v-model="tempData.certNo"
                          placeholder="ดึงมาจาก GAP"
                          variant="outlined"
                          rounded="lg"
                          density="comfortable"
                          hide-details
                      ...
                      <v-col cols="12" class="pa-4">
                          ...
                          >
                            ...
                            <tbody>
                              <tr>
                                ...
                                <td class="pa-2">
                                  <v-text-field
                                    placeholder="ระบุแผนการเพาะปลูก"
                                    variant="outlined"
                                    rounded="lg"
                                    density="compact"
                                    hide-details
                        ...
                        <v-card variant="outlined" class="pa-6 rounded-xl">
                          <v-row dense>
                            ...
                            <v-col cols="12" md="6">
                              <div class="field-label mb-1">
                                ขนาดพื้นที่ปลูก (ไร่) <span class="req">*</span>
                              </div>
                              <v-text-field
                            ...
                            <v-col cols="12">
                              ...
                              <v-text-field
                                placeholder="ระบุที่อยู่ตั้งแปลง"
                                variant="outlined"
                                rounded="lg"
                                density="comfortable"
                                hide-details
                      ...
                      <v-col cols="12" class="pa-4">
                        ...
                        >
                          ...
                          <tbody>
                            ...
                            <tr
                              v-for="(doc, index) in attachmentList"
                              :key="index"
                            >
                              <td class="pa-4">
                              ...
                              <td class="pa-2">
                                <v-file-input
                                  v-model="tempData.attachments[doc.key]"
                                  label="เลือกไฟล์ (PDF, JPG, PNG)"
                                  variant="outlined"
                                  rounded="lg"
                                  density="compact"
                                  hide-details
                              ...
                              <td class="pa-2">
                                <v-select
                                  v-model="tempData.attachmentCheckDoc[doc.key]"
                                  placeholder="ผลการตรวจเอกสาร..."
                                  variant="outlined"
                                  rounded="lg"
                                  density="compact"
                                  hide-details
                            ...
                            <tr class="bg-grey-lighten-4">
                              <td
                                colspan="4"
                                class="pa-3 font-weight-bold text-body-2"
                              >
                            ...
                            <!-- รายการย่อยของข้อ 5 -->
                            <tr
                              v-for="(sub, sIdx) in controlPlanList"
                              :key="'sub-' + sIdx"
                            >
                              <td class="pa-4 pl-8">
                                <div class="d-flex align-start">
                              ...
                              <td class="pa-2">
                                <v-file-input
                                  v-model="tempData.attachments[sub.key]"
                                  variant="outlined"
                                  rounded="lg"
                                  density="compact"
                                  hide-details
                              ...
                              <td class="pa-2">
                                <v-select
                                  v-model="tempData.attachmentCheckDoc[sub.key]"
                                  placeholder="ผลการตรวจเอกสาร..."
                                  variant="outlined"
                                  rounded="lg"
                                  density="compact"
                            ...
                            <tr>
                              ...
                              <td class="pa-2">
                                <v-select
                                  v-model="
                                    tempData.attachmentCheckDoc['edu_evidence']
                                  "
                                  placeholder="ผลการตรวจเอกสาร..."
                                  variant="outlined"
                                  rounded="lg"
                              ...
                              <td class="pa-2">
                                <v-textarea
                                  v-model="
                                    tempData.attachmentRemarks['edu_evidence']
                                  "
                                  placeholder="ระบุหมายเหตุ..."
                                  variant="outlined"
                                  rounded="lg"
                    ...
                    <template v-else>
                      ...
                      <v-col cols="12" md="6">
                        <div class="field-label mb-1">
                          ประเภทการรับรอง <span class="req">*</span>
                        </div>
                        <v-select
                          v-model="tempData.nonGapType"
                          :items="['GAP', 'อื่นๆ']"
                          variant="outlined"
                          rounded="lg"
                          density="comfortable"
            ...
            <v-row v-else-if="editType === 'factory'">
              ...
              <v-col cols="12" md="6">
                ...
                <v-autocomplete
                  v-model="tempData.cropName"
                  :items="['มังคุด', 'ลำไย', 'ทุเรียน']"
                  placeholder="เลือกชนิดพืช (Master Data)"
                  variant="outlined"
                  rounded="lg"
                  density="comfortable"
                  hide-details
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td class="pa-4">
                        <div class="field-label mb-2">
                          <span class="req">*</span> หมายเลข GMP :
                        </div>
                        <v-text-field
                          v-model="tempData.gmpNo"
                          variant="outlined"
                          rounded="lg"
                          density="compact"
                          class="mb-2"
                        ...
                        <v-row dense>
                          ...
                          <v-col cols="6">
                            ...
                            <v-text-field
                              v-model="tempData.gmpExpireDate"
                              type="date"
                              variant="outlined"
                              rounded="lg"
                              density="compact"
                      ...
                      <td class="pa-2 vertical-top">
                        <v-file-input
                          v-model="tempData.attachments['gmp_cert']"
                          variant="outlined"
                          rounded="lg"
                          density="compact"
                          hide-details
                      ...
                      <td class="pa-2 vertical-top">
                        <v-select
                          v-model="tempData.attachmentCheckDoc['gmp_cert']"
                          placeholder="ผลการตรวจเอกสาร..."
                          variant="outlined"
                          rounded="lg"
                          density="compact"
                      ...
                      <td class="pa-2 vertical-top">
                        <v-textarea
                          v-model="tempData.attachmentRemarks['gmp_cert']"
                          placeholder="ระบุหมายเหตุ..."
                          variant="outlined"
                          rounded="lg"
                          density="compact"
                    ...
                    <tr>
                      <td class="pa-4 border-top">
                        ...
                        <v-row dense>
                          <v-col cols="6">
                            ...
                            <v-text-field
                              v-model="tempData.haccpIssueDate"
                              type="date"
                              variant="outlined"
                              rounded="lg"
                              density="compact"
                          ...
                          <v-col cols="6">
                            ...
                            <v-text-field
                              v-model="tempData.haccpExpireDate"
                              type="date"
                              variant="outlined"
                              rounded="lg"
                              density="compact"
                      ...
                      <td class="pa-2 vertical-top border-top">
                        <v-file-input
                          v-model="tempData.attachments['haccp_cert']"
                          variant="outlined"
                          rounded="lg"
                          density="compact"
                          hide-details
                      ...
                      <td class="pa-2 vertical-top">
                        <v-select
                          v-model="tempData.attachmentCheckDoc['haccp_cert']"
                          placeholder="ผลการตรวจเอกสาร..."
                          variant="outlined"
                          rounded="lg"
                          density="compact"
                      ...
                      <td class="pa-2 vertical-top border-top">
                        <v-textarea
                          v-model="tempData.attachmentRemarks['haccp_cert']"
                          placeholder="ระบุหมายเหตุ..."
                          variant="outlined"
                          rounded="lg"
                          density="compact"
...
const farmers = ref([
  {
    id: 1,
    name: "นายเกษตร มั่นคง",
    idNo: "1-1001-00223-34-4",
    addressNo: "1/2",
    province: "กรุงเทพฯ",
  },
]);
const farms = ref([
  {
    id: 1,
    farmerName: "นายเกษตร มั่นคง",
    cropName: "ทุเรียน",
    certNo: "GAP-2568-12345",
    location: "แปลง 1",
  },
]);
const factories = ref([
...
// ─── HEADERS ───
const farmerHeaders = [
  { title: "ชื่อเกษตรกร", key: "name" },
  { title: "เลขบัตรประชาชน", key: "idNo" },
  { title: "บ้านเลขที่", key: "addressNo" },
  { title: "จังหวัด", key: "province" },
  { title: "แก้ไข", key: "actions", sortable: false, align: "center" },
];
const farmHeaders = [
  { title: "ชื่อเกษตรกร", key: "farmerName" },
  { title: "ชื่อพืช", key: "cropName" },
  { title: "รหัสใบรับรอง", key: "certNo" },
  { title: "แก้ไข", key: "actions", sortable: false, align: "center" },
];
const factoryHeaders = [
  { title: "ชื่อพืช", key: "cropName" },
  { title: "หมายเลข GMP", key: "gmpNo" },
...
function goToCreatePage(type) {
  ...
  Object.keys(tempData).forEach((key) => {
    if (
      key === "attachments" ||
      key === "attachmentRemarks" ||
      key === "attachmentCheckDoc"
    ) {
...
function removeItems(type) {
  if (type === "farmer") {
    farmers.value = farmers.value.filter(
      (f) => !selectedFarmerRows.value.includes(f.id),
    );
    selectedFarmerRows.value = [];
  } else if (type === "farm") {
    farms.value = farms.value.filter(
      (f) => !selectedFarmRows.value.includes(f.id),
    );
    selectedFarmRows.value = [];
  } else {
    factories.value = factories.value.filter(
      (f) => !selectedFactoryRows.value.includes(f.id),
    );
...
function addFarmerFromMaster() {
  if (selectedMasterFarmer.value) {
    const masterData = masterFarmers.find(
      (f) => f.name === selectedMasterFarmer.value,
    );
    if (masterData)
      farmers.value.push({
        id: Date.now(),
...
function addFactoryFromMaster() {
  if (selectedMasterFactory.value) {
    ...
    const masterData = masterFactories.find(
      (f) => f.id === selectedMasterFactory.value,
    );
...
const uploadSections = [
  {
    ...
    files: ["แผนที่แปลง", "สำเนาใบรับรอง GAP"],
  },
  {
...
const attachmentList = [
  ...
  {
    key: "farm_contract",
    label: "3. สัญญาประทุษเกษตรและโรงคัดบรรจุ (Farm Contract) :",
    required: true,
  },
  {
    key: "chemical_list",
    label: "4. บัญชีรายชื่อสารเคมีที่ใช้ในการปลูกพืชแต่ละชนิด :",
    required: true,
  },
];
...
const controlPlanList = [
  {
    key: "visit_plan",
    label: "แผนการบันทึกการตรวจเยี่ยม/ตรวจติดตามแปลงเกษตรกร",
    required: true,
  },
  {
    key: "control_record",
    label: "แผนการควบคุม/บันทึกการตรวจเยี่ยม/บันทึกเกษตรกร",
    required: true,
  },
  {
    key: "chemical_use",
    label: "สารเคมี/ปุ๋ย/ฮอร์โมน ที่ใช้ในแปลงปลูก",
    required: true,
  },
  {
    key: "tools_organic",
    label: "เครื่องมือ/วัสดุอุปกรณ์การเกษตร (กรณีการผลิตพืชอินทรีย์)",
    required: false,
  },
  {
    key: "harvest_manage",
    label: "การเก็บเกี่ยวและการจัดการหลังเก็บเกี่ยว",
    required: true,
  },
  {
    key: "lab_result",
    label: "ผลวิเคราะห์สารตกค้าง/เชื้อจุลินทรีย์",
    required: true,
  },
...
const factoryDocList = [
  {
    key: "prod_plan",
    label: "2. แผนการผลิตพืช และกำลังการผลิตพืชต่อวัน/สัปดาห์/เดือน :",
  },
  {
    key: "vendor_list",
    label: "3. แผนการควบคุมกระบวนการผลิต (Approve Vendor List : AVL) :",
  },
  {
    key: "safety_plan",
    label: " - แผนการควบคุมระบบความปลอดภัยของพืช (สารเคมี/จุลินทรีย์) :",
  },
  {
    key: "wash_process",
    label: " - ขั้นตอนการล้าง และแบบฟอร์มการบันทึกการล้าง :",
  },
  {
    key: "recall_plan",
    label: " - การชี้บ่งและการตรวจสอบย้อนกลับ (Mock recall) :",
  },
