  ...
  <div>
    <div class="d-flex align-center justify-space-between flex-wrap ga-3 mb-6">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis mb-0">ภาพรวมระบบออกใบรับรองสุขอนามัย (Health Certificate) ตามประกาศพืชควบคุมเฉพาะ</p>
      </div>
    </div>
    ...
    <v-row>
      ...
      <v-col cols="12" md="8">
        <v-card rounded="xl" elevation="0">
          <v-card-title class="d-flex align-center ga-2 pa-4 pb-3">
            ...
            <v-spacer />
            <v-btn variant="text" color="hc-staff" size="small" append-icon="fas fa-arrow-right" @click="router.push('/hc/staff/applications')">
              ดูทั้งหมด
            </v-btn>
          </v-card-title>
          <v-divider />
          <v-list lines="two" class="pa-0">
            <template v-for="(app, i) in recentApplications" :key="app.id">
              <v-list-item class="pa-3" @click="router.push(`/hc/staff/applications/${app.id}`)">
                <template #prepend>
                  <v-avatar :color="getStatusColor(app.status)" variant="tonal" size="40" rounded="lg" class="mr-3">
                ...
                <v-list-item-subtitle class="text-caption">{{ app.exporter }} · {{ app.type }}</v-list-item-subtitle>
                <template #append>
                  <div class="d-flex flex-column align-end ga-1">
                    <v-chip :color="getStatusColor(app.status)" size="x-small" variant="tonal">{{ getStatusLabel(app.status) }}</v-chip>
      ...
      <v-col cols="12" md="4">
        <v-alert color="hc-staff" variant="tonal" rounded="xl" class="mb-4" prepend-icon="fas fa-circle-info">
          <div class="text-body-2 font-weight-medium mb-1">สรุปงานที่รอดำเนินการ</div>
          <div class="text-body-2">มีคำขอรอดำเนินการ 12 รายการ รวมถึงรอตรวจ Lab 8 รายการ</div>
        </v-alert>
        <!-- Status Summary -->
...
const stats = [
  { label: "คำขอทั้งหมด", value: "148", icon: "fas fa-file-lines", iconColor: "hc-staff" },
  { label: "รอตรวจสอบ", value: "12", icon: "fas fa-magnifying-glass", iconColor: "warning" },
  { label: "ตรวจ Lab", value: "8", icon: "fas fa-flask", iconColor: "warning" },
  { label: "รอพิจารณา", value: "5", icon: "fas fa-gavel", iconColor: "warning" },
  { label: "รอลงนาม", value: "4", icon: "fas fa-signature", iconColor: "warning" },
  { label: "ออกใบรับรองแล้ว", value: "119", icon: "fas fa-file-shield", iconColor: "success" },
];

const recentApplications = [
  { id: "HC-001", requestNo: "HC-2568-00041", exporter: "บ.ไทยฟรุ๊ต จก.", type: "ขอใบรับรอง", submittedAt: "15 ม.ค. 68", status: "under_review" },
  { id: "HC-002", requestNo: "HC-2568-00039", exporter: "บ.สยามเอ็กซ์พอร์ต", type: "ขอใบรับรอง", submittedAt: "13 ม.ค. 68", status: "testing" },
  { id: "HC-003", requestNo: "HC-2568-00036", exporter: "บ.กรีนเฟรช จก.", type: "แก้ไขใบรับรอง", submittedAt: "10 ม.ค. 68", status: "pending_approval" },
  { id: "HC-004", requestNo: "HC-2568-00034", exporter: "บ.ดีเอ็กซ์พอร์ต", type: "ขอใบรับรอง", submittedAt: "8 ม.ค. 68", status: "approved" },
  { id: "HC-005", requestNo: "HC-2568-00030", exporter: "บ.ไทยอะกริ จก.", type: "ขอใบรับรอง", submittedAt: "3 ม.ค. 68", status: "completed" },
];
