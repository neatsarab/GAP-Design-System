      ...
      <v-card-text class="pa-4">
        <v-row dense>
          ...
          <v-col cols="12" sm="1" class="d-flex align-center">
            <v-btn variant="tonal" color="grey" block @click="clearFilters">
      ...
      >
        ...
        <template #item.actions="{ item }">
          <v-btn
            ...
            @click.stop="router.push(`/officer/applications/${item.id}`)"
          >
            ดูรายละเอียด
          </v-btn>
        </template>
...
function onRowClick(_e: unknown, row: { item: ApplicationRow }) {
  router.push(`/officer/applications/${row.item.id}`)
}
