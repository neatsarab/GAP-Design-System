<template>
  <div>
    <div class="d-flex align-center justify-space-between mb-5 flex-wrap ga-3">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis mb-0">
          จัดการคำขอรับรองมาตรฐาน GAP สำหรับกลุ่มเกษตรกร
        </p>
      </div>
      <v-btn color="gap-staff" prepend-icon="fas fa-users-plus"
        >ยื่นคำขอกลุ่มใหม่</v-btn
      >
    </div>
      ...
      <v-card-text class="pa-4">
        <v-row dense>
          <v-col cols="12" sm="5">
            <v-text-field
              v-model="search"
              label="ค้นหาชื่อกลุ่ม / เลขที่คำขอ"
          ...
          <v-card-text class="pa-5">
            <div class="d-flex align-start justify-space-between mb-3">
              ...
              <v-chip
                ...
                variant="tonal"
                :prepend-icon="getStatusIcon(group.status)"
              >
            ...
            <v-row dense>
              <v-col cols="6"
                ><info-item
                  icon="fas fa-users"
                  label="สมาชิก"
                  :value="`${group.memberCount} ราย`"
              /></v-col>
              <v-col cols="6"
                ><info-item
                  icon="fas fa-map"
                  label="แปลงรวม"
                  :value="`${group.totalArea} ไร่`"
              /></v-col>
              <v-col cols="6"
                ><info-item
              ...
              <v-col cols="6"
                ><info-item
                  ...
                  label="ใบรับรอง"
                  :value="group.certType"
              /></v-col>
              <v-col cols="6"
                ><info-item
                  icon="fas fa-calendar"
                  label="วันที่ยื่น"
                  :value="group.submittedAt"
              /></v-col>
            </v-row>
            ...
            <div class="d-flex align-center ga-2 mb-3">
              <span class="text-caption text-medium-emphasis"
                >ระบบควบคุมภายใน (ICS):</span
              >
              <v-chip
                v-for="ics in group.icsStatus"
                :key="ics.label"
                size="x-small"
    ...
    <v-empty-state
      ...
      icon="fas fa-users"
      headline="ไม่พบรายการ"
      text="ไม่พบคำขอกลุ่มที่ตรงกับเงื่อนไขที่เลือก"
    />
  </div>
</template>
...
const groupApplications = [
  {
    ...
    groupName: "กลุ่มเกษตรกรผู้ปลูกมะม่วงบ้านดอนแก้ว",
    memberCount: 18,
    totalArea: 245,
    crop: "มะม่วง",
    province: "เชียงใหม่",
    ...
    status: "scheduling",
    icsStatus: [
      { label: "นโยบาย", ok: true },
      { label: "ตรวจภายใน", ok: true },
      { label: "ข้อร้องเรียน", ok: false },
    ],
  },
  {
    id: "GRP-002",
    requestNo: "GAP-GRP-2569-00004",
    groupName: "สหกรณ์การเกษตรกลุ่มลำไยเชียงราย",
    memberCount: 32,
    totalArea: 480,
    crop: "ลำไย",
    province: "เชียงราย",
    ...
    icsStatus: [
      { label: "นโยบาย", ok: true },
      { label: "ตรวจภายใน", ok: true },
      { label: "ข้อร้องเรียน", ok: true },
    ],
  },
  {
    id: "GRP-003",
    requestNo: "GAP-GRP-2569-00003",
    groupName: "กลุ่มเกษตรกรพริกเกษตรอินทรีย์สระแก้ว",
    memberCount: 12,
    totalArea: 96,
    crop: "พริก",
    province: "สระแก้ว",
  ...
  },
  {
    ...
    groupName: "กลุ่มผู้ปลูกข้าวโพดเพชรบูรณ์",
    memberCount: 25,
    totalArea: 312,
    crop: "ข้าวโพดหวาน",
    province: "เพชรบูรณ์",
...
const filteredGroups = computed(() => {
  ...
  if (search.value) {
    ...
    items = items.filter(
      (i) => i.groupName.includes(q) || i.requestNo.toLowerCase().includes(q),
    );
  }
  return items;
});
