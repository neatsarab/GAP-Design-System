<template>
  <div>
    <div class="d-flex align-center justify-space-between mb-5 flex-wrap ga-3">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis mb-0">
          รายการใบรับรองแหล่งผลิต GAP พืชทั้งหมดของคุณ
        </p>
      </div>
      <div class="d-flex ga-2">
        <v-btn variant="tonal" color="gap-user" prepend-icon="fas fa-download"
          >ส่งออก Excel</v-btn
        >
      </div>
    </div>
      ...
      <v-card-text class="pa-4">
        <v-row dense align="center">
          <v-col cols="12" sm="6">
            ...
            <v-text-field
              v-model="search"
              placeholder="ค้นหาเลขใบรับรอง / แปลง / พืช"
    ...
    <v-chip-group v-model="filterTab" class="mb-4" mandatory>
      <v-chip value="all" color="gap-user" variant="tonal" filter size="small">
        <v-icon start icon="fas fa-list" size="12" /> ทั้งหมด
        <v-badge :content="certs.length" inline color="gap-user" class="ml-1" />
      </v-chip>
      <v-chip
        value="active"
        color="success"
    ...
    <v-card rounded="xl" elevation="0" class="data-card">
      ...
      >
        <template #item.certNo="{ item }">
          <span class="text-body-2 font-weight-bold text-gap-user">{{
            item.certNo
          }}</span>
        </template>
...
const headers = [
  ...
  { title: "ชื่อแปลง", key: "plotName", width: 160 },
  { title: "ชนิดพืช", key: "cropType", width: 120 },
  { title: "จังหวัด", key: "plotProvince", width: 120 },
