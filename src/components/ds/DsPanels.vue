        ...
        <v-expansion-panel-title>
          <div class="d-flex align-center ga-3">
            ...
            <v-chip size="x-small" color="success" variant="tonal" class="ml-2">
              {{ cat.items.filter(i => i.result === 'PASS').length }}/{{ cat.items.length }} ผ่าน
            </v-chip>
          </div>
        </v-expansion-panel-title>
      ...
      <v-list lines="two">
        <template v-for="(n, i) in notifications" :key="n.id">
          <v-list-item class="px-5 py-3">
            ...
            <v-list-item-title class="text-body-2 font-weight-medium">{{ n.message }}</v-list-item-title>
            <v-list-item-subtitle class="text-caption">{{ n.date }}</v-list-item-subtitle>
            <template #append>
              <v-chip :color="n.chipColor" size="x-small" variant="tonal">{{ n.status }}</v-chip>
            </template>
          </v-list-item>
