<template>
  <v-card>
    <v-card-text class="pa-6">
      <AppStepper v-model="step" :steps="steps" @submit="handleSubmit">
        <!-- Step 1 -->
        <template #step-1>
          <v-row dense>
            <v-col cols="12" sm="4">
        ...
        <template #step-4>
          <div class="text-center py-6">
            ...
            <div class="text-body-2 text-medium-emphasis">
              {{ form.prefix }}{{ form.firstName }} {{ form.lastName }} — แปลง:
              {{ form.plotName || "-" }}
            </div>
          </div>
        </template>
...
const steps = [
  {
    title: "ข้อมูลผู้ขอ",
    subtitle: "ชื่อ-นามสกุล, บัตรประชาชน",
    icon: "fas fa-user",
  },
  {
    title: "ข้อมูลแปลง",
    subtitle: "ชื่อแปลง, พื้นที่, จังหวัด",
    icon: "fas fa-map-location-dot",
  },
  { title: "เอกสารแนบ", subtitle: "PDF, DOC", icon: "fas fa-paperclip" },
  {
    title: "ตรวจสอบ & ยื่น",
    subtitle: "ยืนยันและส่งคำขอ",
