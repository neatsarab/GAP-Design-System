<template>
  <div style="--v-theme-primary: var(--v-theme-cb-staff)">
    <v-navigation-drawer v-model="drawer" :rail="rail" permanent color="surface" class="app-sidebar" :style="{ '--v-theme-primary': 'var(--v-theme-cb-staff)' }">
      <v-list-item nav class="py-4 px-4">
        <template v-slot:prepend>
          <div class="logo-icon-box rounded-lg mr-3" style="background: rgba(var(--v-theme-cb-staff), 0.12); border: 1px solid rgba(var(--v-theme-cb-staff), 0.2);">
            <v-icon icon="fas fa-certificate" color="cb-staff" size="20" />
          </div>
        </template>
        <v-list-item-title class="text-body-2 font-weight-bold">ระบบการขึ้นทะเบียนหน่วยรับรองโรงงานผลิตสินค้าพืช</v-list-item-title>
        <v-list-item-subtitle class="text-caption" style="color: rgb(var(--v-theme-cb-staff)); opacity: 0.85">สำหรับเจ้าหน้าที่</v-list-item-subtitle>
      ...
      <div v-if="!rail" class="px-4 mb-2">
        <div class="user-card rounded-lg pa-3 d-flex align-center ga-2" style="background: rgba(var(--v-theme-cb-staff), 0.06); border: 1px solid rgba(var(--v-theme-cb-staff), 0.12);">
          <v-avatar color="cb-staff" size="32" variant="tonal">
            <v-icon icon="fas fa-user-tie" size="16" color="cb-staff" />
          </v-avatar>
          <div class="flex-grow-1 overflow-hidden">
            <div class="text-truncate text-body-2 font-weight-medium text-cb-staff">นายประพัฒน์ ตรวจสอบ</div>
            <div class="text-caption text-medium-emphasis">กวก. / นักวิชาการเกษตร</div>
          </div>
        </div>
      </div>
      ...
      <v-list density="compact" nav class="mt-1 px-2">
        <template v-for="group in navGroups" :key="group.label">
          ...
          <v-list-item v-for="item in group.items" :key="item.to" :prepend-icon="item.icon" :title="item.title" :to="item.to" :active="isNavActive(item.to)" active-color="cb-staff" rounded="lg" class="mb-1">
    ...
    <v-app-bar flat height="64" class="app-bar" :style="{ '--v-theme-primary': 'var(--v-theme-cb-staff)' }">
      ...
      <div class="d-flex align-center ga-1 mr-3">
        <v-btn variant="text" size="small" icon class="mr-1">
          <v-badge color="error" content="5" floating>
            <v-icon icon="fas fa-bell" size="20" color="cb-staff" />
          </v-badge>
        </v-btn>
        ...
        <v-chip variant="outlined" color="cb-staff" class="user-chip mr-2 ml-1" prepend-icon="fas fa-user-tie">นายประพัฒน์ ตรวจสอบ</v-chip>
      </div>
    </v-app-bar>
    ...
    <v-main class="bg-background">
      <v-container fluid class="pa-5 pa-md-7" style="max-width: 1320px; --v-theme-primary: var(--v-theme-cb-staff)">
        <router-view />
      </v-container>
    </v-main>
  </div>
...
const breadcrumbs = computed(() => [
  { title: "ระบบการขึ้นทะเบียนหน่วยรับรองโรงงานผลิตสินค้าพืช", to: "/cb/staff" },
...
const navGroups = [
  {
    ...
    divider: true,
    items: [
      { title: "แดชบอร์ด", icon: "fas fa-gauge", to: "/cb/staff/dashboard" },
    ],
  },
  {
    label: "รายการคำขอ",
    divider: true,
    items: [
      { title: "รายการคำขอ", icon: "fas fa-file-lines", to: "/cb/staff/applications", count: 3 },
    ],
  },
  {
    label: "ข้อมูลทะเบียน",
    divider: true,
    items: [
      { title: "ข้อมูลทะเบียน", icon: "fas fa-certificate", to: "/cb/staff/registration-data" },
    ],
  },
];
</script>
