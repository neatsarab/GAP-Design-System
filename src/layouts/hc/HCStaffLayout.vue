<template>
  <div style="--v-theme-primary: var(--v-theme-hc-staff)">
    ...
    >
      ...
      <v-list-item nav class="py-4 px-4">
        <template v-slot:prepend>
          <div class="logo-icon-box rounded-lg mr-3" style="background: rgba(var(--v-theme-hc-staff), 0.12); border: 1px solid rgba(var(--v-theme-hc-staff), 0.2);">
        ...
        <v-list-item-subtitle
          class="text-caption"
          style="color: rgb(var(--v-theme-hc-staff)); opacity: 0.85"
      ...
      <div v-if="!rail" class="px-4 mb-2">
        <div class="user-card rounded-lg pa-3 d-flex align-center ga-2" style="background: rgba(var(--v-theme-hc-staff), 0.06); border: 1px solid rgba(var(--v-theme-hc-staff), 0.12);">
          <v-avatar color="hc-staff" size="32">
    ...
    <!-- ── App Bar ── -->
    <v-app-bar flat height="64" class="app-bar" :style="{ '--v-theme-primary': 'var(--v-theme-hc-staff)' }">
      <v-btn
        icon="fas fa-bars"
        variant="text"
        size="small"
      ...
      <div class="d-flex align-center ga-1 mr-3">
        ...
        <v-btn variant="text" size="small" icon class="mr-1">
          <v-badge color="error" content="3" floating>
            <v-icon icon="fas fa-bell" size="20" color="hc-staff" />
          </v-badge>
        </v-btn>
    ...
    <v-main class="bg-background">
      <v-container fluid class="pa-5 pa-md-7" style="max-width: 1320px; --v-theme-primary: var(--v-theme-hc-staff)">
        <router-view />
      </v-container>
    </v-main>
  </div>
...
const navGroups = [
  {
    ...
    divider: true,
    items: [
      { title: "แดชบอร์ด", icon: "fas fa-gauge", to: "/hc/staff/dashboard" },
    ],
  },
  {
    label: "รายการคำขอ",
