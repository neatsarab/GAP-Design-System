<template>
  <div style="--v-theme-primary: var(--v-theme-export-user)">
    <v-navigation-drawer v-model="drawer" :rail="rail" permanent color="surface" class="app-sidebar" :style="{ '--v-theme-primary': 'var(--v-theme-export-user)' }">
      <v-list-item nav class="py-4 px-4">
        <template v-slot:prepend>
          <div class="logo-icon-box rounded-lg mr-3" style="background: rgba(var(--v-theme-export-user), 0.12); border: 1px solid rgba(var(--v-theme-export-user), 0.2);">
      ...
      <div v-if="!rail" class="px-4 mb-2">
        <div class="user-card rounded-lg pa-3 d-flex align-center ga-2" style="background: rgba(var(--v-theme-export-user), 0.06); border: 1px solid rgba(var(--v-theme-export-user), 0.12);">
          <v-avatar color="export-user" size="32" variant="tonal">
            <v-icon :icon="sessionStore.entityIcon" size="16" color="export-user" />
          </v-avatar>
          <div class="flex-grow-1 overflow-hidden">
            <div class="text-truncate text-body-2 font-weight-medium text-export-user">{{ sessionStore.displayName }}</div>
    ...
    <v-app-bar flat height="64" class="app-bar" :style="{ '--v-theme-primary': 'var(--v-theme-export-user)' }">
    ...
    <v-main class="bg-background">
      <v-container fluid class="pa-5 pa-md-7" style="max-width: 1320px; --v-theme-primary: var(--v-theme-export-user)">
        <router-view />
      </v-container>
    </v-main>
  </div>
