    ...
    <v-navigation-drawer v-model="drawer" :rail="rail" permanent color="surface" class="app-sidebar">
      ...
      <v-list density="compact" nav class="mt-1 px-2">
        <template v-for="group in navGroups" :key="group.label">
          ...
          <v-list-item v-for="item in group.items" :key="item.to" :prepend-icon="item.icon" :title="item.title" :to="item.to" active-color="org-staff" rounded="lg" class="mb-1" />
...
const breadcrumbs = computed(() => [
  { title: "ระบบ ORG (เจ้าหน้าที่)", to: "/org/staff" },
...
const navGroups = [
  ...
  },
  {
    ...
    divider: true,
    items: [
      { title: "รายการคำขอ", icon: "fas fa-file-lines", to: "/org/staff/applications" },
      { title: "นัดตรวจแปลง", icon: "fas fa-calendar-check", to: "/org/staff/schedule" },
    ],
  },
];
</script>
...
.logo-icon-box { width: 36px; height: 36px; background: rgba(var(--v-theme-org-staff), 0.12); border: 1px solid rgba(var(--v-theme-org-staff), 0.2); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.user-card { background: rgba(var(--v-theme-org-staff), 0.06); border: 1px solid rgba(var(--v-theme-org-staff), 0.12); }
...
.user-chip { font-size: 12px; background: rgba(var(--v-theme-org-staff), 0.06); }
