<template>
    <div style="--v-theme-primary: var(--v-theme-org-staff)">
        <v-navigation-drawer v-model="drawer" :rail="rail" permanent color="surface" class="app-sidebar"
            :style="{ '--v-theme-primary': 'var(--v-theme-org-staff)' }">
            <v-list-item nav class="py-4 px-4">
              ...
              border: 1px solid rgba(var(--v-theme-org-staff), 0.2);
            ">
                        <v-icon icon="fas fa-leaf" color="org-staff" size="20" />
                    </div>
                </template>
                <v-list-item-title class="text-body-2 font-weight-bold"
                    style="word-break: break-word; white-space: normal">ระบบการรับรองมาตรฐานเกษตรอินทรีย์</v-list-item-title>
                <v-list-item-subtitle class="text-caption"
                    style="color: rgb(var(--v-theme-org-staff)); opacity: 0.85">สำหรับเจ้าหน้าที่</v-list-item-subtitle>
            ...
            background: rgba(var(--v-theme-org-staff), 0.06);
            border: 1px solid rgba(var(--v-theme-org-staff), 0.12);
          ">
                    <v-avatar color="org-staff" size="32" variant="tonal">
                        <v-icon icon="fas fa-user-tie" size="16" color="org-staff" />
                    </v-avatar>
                    <div class="flex-grow-1 overflow-hidden">
                        <div class="text-truncate text-body-2 font-weight-medium text-org-staff">
            ...
            <v-list density="compact" nav class="mt-1 px-2">
                <template v-for="group in navGroups" :key="group.label">
                    ...
                    <v-list-item v-for="item in group.items" :key="item.to" :prepend-icon="item.icon"
                        :title="item.title" :to="item.to" :active="isNavActive(item.to)" active-color="org-staff"
            ...
            <div class="d-flex align-center ga-1 mr-3">
                ...
                <v-menu location="bottom end" :offset="8">
                    <template #activator="{ props }">
                        <v-chip v-bind="props" variant="outlined" color="org-staff" class="user-chip mr-2 ml-1"
        ...
        <v-main class="bg-background">
            <v-container fluid class="pa-5 pa-md-7"
                style="max-width: 1320px; --v-theme-primary: var(--v-theme-org-staff)">
                <router-view />
            </v-container>
        </v-main>
    </div>
    ...
    //   },
    {
        ...
        items: [
            ...
            },
            {
                ...
                to: "/org/staff/propose-cc",
            },
            {
                title: "บันทึกครั้งที่เสนอ CC",
                icon: "fas fa-gavel",
                to: "/org/staff/cc-results",
            },
            {
                title: "บันทึกผลจาก CC",
                icon: "fas fa-gavel",
                to: "/org/staff/cc-saveresults",
            },
        ],
    },
    {
        ...
        items: [
            {
                ...
                icon: "fas fa-file-pen",
                to: "/org/staff/datatransfer/farmer-data-doae",
            },
            {
                title: "ข้อมูลพื้นที่ทำการเกษตร (ส.ป.ก.)",
                icon: "fas fa-file-lines",
                to: "/org/staff/datatransfer/farm-land-data-kps",
            },
            {
                title: "ข้อมูลตรวจสอบมาตรฐาน (มกอช.)",
                icon: "fas fa-file-pen",
                to: "/org/staff/datatransfer/standard-inspection-data",
            },
            {
                title: "Bizportal",
                icon: "fas fa-file-pen",
                to: "/org/staff/datatransfer/bizportal",
            },
        ...
        items: [
            {
                ...
                to: "/org/staff/registry",
            },
            {
                title: "รายการใบทะเบียน",
                icon: "fas fa-certificate",
                to: "/org/staff/registrylist",
            },
        ],
    },
    // {
...
<style scoped>
.user-avatar-md {
    background: rgba(var(--v-theme-org-staff), 0.15);
