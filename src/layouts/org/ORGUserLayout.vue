  ...
  <div style="--v-theme-primary: var(--v-theme-org-user)">
    <v-navigation-drawer v-model="drawer" :rail="rail" permanent color="surface" class="app-sidebar">
      <v-list-item nav class="py-4 px-4">
        ...
        <v-list-item-title class="text-body-2 font-weight-bold">ระบบ ORG เกษตรอินทรีย์</v-list-item-title>
        <v-list-item-subtitle class="text-caption" style="color: rgb(var(--v-theme-org-user)); opacity: 0.85">สำหรับเกษตรกร / ผู้ประกอบการ</v-list-item-subtitle>
      ...
      <div v-if="!rail" class="px-4 mb-2">
        <div class="user-card rounded-lg pa-3 d-flex align-center ga-2">
          <v-avatar color="org-user" size="32" variant="tonal">
            <v-icon icon="fas fa-user" size="16" color="org-user" />
          </v-avatar>
          <div class="flex-grow-1 overflow-hidden">
            <div class="text-truncate text-body-2 font-weight-medium text-org-user">สมชาย ใจดี</div>
            <div class="text-caption text-medium-emphasis">เลขบัตรประชาชน : 1-1000-00000-00-0</div>
          </div>
        </div>
      </div>
      ...
      <v-list density="compact" nav class="mt-1 px-2">
        <template v-for="group in navGroups" :key="group.label">
          ...
          <v-list-item v-for="item in group.items" :key="item.to" :prepend-icon="item.icon" :title="item.title" :to="item.to" active-color="org-user" rounded="lg" class="mb-1" />
    ...
    <v-app-bar flat height="64" class="app-bar">
      ...
      <div class="d-flex align-center ga-1 mr-3">
        ...
        <v-chip variant="outlined" color="org-user" class="user-chip mr-2 ml-1" prepend-icon="fas fa-user">สมชาย ใจดี</v-chip>
      </div>
    </v-app-bar>
...
const navGroups = [
  {
    ...
    divider: true,
    items: [
      { title: "แดชบอร์ด", icon: "fas fa-gauge", to: "/org/user/dashboard" },
    ],
  },
  {
    label: "การยื่นคำขอ",
    divider: true,
    items: [
      { title: "ยื่นคำขอใหม่", icon: "fas fa-file-pen", to: "/org/user/applications/new" },
      { title: "รายการคำขอ", icon: "fas fa-file-lines", to: "/org/user/applications" },
    ],
  },
];
</script>
...
.logo-icon-box { width: 36px; height: 36px; background: rgba(var(--v-theme-org-user), 0.12); border: 1px solid rgba(var(--v-theme-org-user), 0.2); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.user-card { background: rgba(var(--v-theme-org-user), 0.06); border: 1px solid rgba(var(--v-theme-org-user), 0.12); }
...
.user-chip { font-size: 12px; background: rgba(var(--v-theme-org-user), 0.06); }
