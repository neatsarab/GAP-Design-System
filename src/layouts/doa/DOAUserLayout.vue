  ...
  <div style="--v-theme-primary: var(--v-theme-doa-user)">
    <v-navigation-drawer v-model="drawer" :rail="rail" permanent color="surface" class="app-sidebar">
      <v-list-item nav class="py-4 px-4">
        ...
        <v-list-item-subtitle class="text-caption" style="color: rgb(var(--v-theme-doa-user)); opacity: 0.85">สำหรับผู้ประกอบการโรงงาน</v-list-item-subtitle>
      ...
      <div v-if="!rail" class="px-4 mb-2">
        <div class="user-card rounded-lg pa-3 d-flex align-center ga-2">
          <v-avatar color="doa-user" size="32" variant="tonal">
            <v-icon icon="fas fa-building" size="16" color="doa-user" />
          </v-avatar>
          <div class="flex-grow-1 overflow-hidden">
            <div class="text-truncate text-body-2 font-weight-medium text-doa-user">บ.ไทยแลนด์ โปรดักส์ จก.</div>
            <div class="text-caption text-medium-emphasis">เลขนิติบุคคล : 1100090000088</div>
          </div>
        </div>
      </div>
      ...
      <v-list density="compact" nav class="mt-1 px-2">
        <template v-for="group in navGroups" :key="group.label">
          ...
          <v-list-item v-for="item in group.items" :key="item.to" :prepend-icon="item.icon" :title="item.title" :to="item.to" active-color="doa-user" rounded="lg" class="mb-1" />
    ...
    <v-app-bar flat height="64" class="app-bar">
      ...
      <div class="d-flex align-center ga-1 mr-3">
        ...
        <v-chip variant="outlined" color="doa-user" class="user-chip mr-2 ml-1" prepend-icon="fas fa-building">บ.ไทยแลนด์ โปรดักส์ จก.</v-chip>
      </div>
    </v-app-bar>
...
const breadcrumbs = computed(() => [
  { title: "ระบบ DOA (ผู้ประกอบการ)", to: "/doa/user" },
...
const navGroups = [
  ...
  },
  {
    ...
    divider: true,
    items: [
      { title: "ยื่นคำขอใหม่", icon: "fas fa-file-pen", to: "/doa/user/applications/new" },
      { title: "รายการคำขอ", icon: "fas fa-file-lines", to: "/doa/user/applications" },
    ],
  },
  {
    label: "ใบรับรอง",
    divider: false,
    items: [
      { title: "ใบรับรอง", icon: "fas fa-certificate", to: "/doa/user/certificates" },
    ],
  },
];
</script>
