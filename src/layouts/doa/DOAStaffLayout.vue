<template>
  <div style="--v-theme-primary: var(--v-theme-doa-staff)">
    <v-navigation-drawer v-model="drawer" :rail="rail" permanent color="surface" class="app-sidebar" :style="{ '--v-theme-primary': 'var(--v-theme-doa-staff)' }">
      <v-list-item nav class="py-4 px-4">
        <template v-slot:prepend>
          <div class="logo-icon-box rounded-lg mr-3" style="background: rgba(var(--v-theme-doa-staff), 0.12); border: 1px solid rgba(var(--v-theme-doa-staff), 0.2);">
      ...
      <div v-if="!rail" class="px-4 mb-2">
        <div class="user-card rounded-lg pa-3 d-flex align-center ga-2" style="background: rgba(var(--v-theme-doa-staff), 0.06); border: 1px solid rgba(var(--v-theme-doa-staff), 0.12);">
          <v-avatar color="doa-staff" size="32" variant="tonal">
            <v-icon icon="fas fa-user-tie" size="16" color="doa-staff" />
          </v-avatar>
          <div class="flex-grow-1 overflow-hidden">
            <div class="text-truncate text-body-2 font-weight-medium text-doa-staff">นายวิชัย เกษตรกรรม</div>
    ...
    <v-app-bar flat height="64" class="app-bar" :style="{ '--v-theme-primary': 'var(--v-theme-doa-staff)' }">
      ...
      <div class="d-flex align-center ga-1 mr-3">
        ...
        <v-chip variant="outlined" color="doa-staff" class="user-chip mr-2 ml-1" prepend-icon="fas fa-user-tie">นายวิชัย เกษตรกรรม</v-chip>
      </div>
    </v-app-bar>
    ...
    <v-main class="bg-background">
      <v-container fluid class="pa-5 pa-md-7" style="max-width: 1320px; --v-theme-primary: var(--v-theme-doa-staff)">
        <router-view />
      </v-container>
    </v-main>
  </div>
...
const navGroups = [
  {
    ...
    divider: true,
    items: [
      { title: "ข้อมูลภาพรวม", icon: "fas fa-gauge", to: "/doa/staff/dashboard" },
    ],
  },
  {
    label: "คำขอ",
  ...
  },
  {
    ...
    divider: false,
    items: [
      { title: "ขอบข่ายมาตรฐาน", icon: "fas fa-certificate", to: "/doa/staff/master/standard-scopes" },
      { title: "ผลิตภัณฑ์", icon: "fas fa-box-open", to: "/doa/staff/master/products" },
    ],
  },
];
</script>
